...
public class Pack implements Iterable<PackIndex.MutableEntry> {
	 ...
	 */
	public void close() {
		//System.out.println("Time:"+java.time.LocalTime.ofNanoOfDay(java.lang.System.currentTimeMillis() % 86_400_000 * 1_000_000L).format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss,SSS"))+"; Method: Pack.close(); Pack file: "+this.getPackFile().getName());
		WindowCache.purge(this);
		closeIndices();
	}

	private synchronized void closeIndices() {
		//System.out.println("Time:"+java.time.LocalTime.ofNanoOfDay(java.lang.System.currentTimeMillis() % 86_400_000 * 1_000_000L).format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss,SSS"))+"; Method: Pack.closeIndices(); Pack file: "+this.getPackFile().getName());
		loadedIdx.clear();
		reverseIdx.clear();
		bitmapIdx.clear();
	}
	...
	boolean endWindowCache() {
		synchronized (activeLock) {
			...
			//System.out.println("Time:"+java.time.LocalTime.ofNanoOfDay(java.lang.System.currentTimeMillis() % 86_400_000 * 1_000_000L).format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss,SSS"))+"; Method: Pack.endWindowCache; activeWindow is  "+activeWindows+"; Pack file is: "+this.getPackName());
			if (r && activeCopyRawData == 0) {
				//System.out.println("Time:"+java.time.LocalTime.ofNanoOfDay(java.lang.System.currentTimeMillis() % 86_400_000 * 1_000_000L).format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss,SSS"))+"; Method: Pack.endWindowCache; activeWindow is  "+activeWindows+"; DoClose being executed.");
				doClose();
			}
			return r;
		}
	...
	private void doOpen() throws IOException {
		...
		try {
			synchronized (readLock) {
				...
				length = fd.length();
				onOpenPack();
				//System.out.println("Time:"+java.time.LocalTime.ofNanoOfDay(java.lang.System.currentTimeMillis() % 86_400_000 * 1_000_000L).format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss,SSS"))+"; Method: Pack.doOpen(); packed file  "+this.getPackName());
			}
		} catch (InterruptedIOException e) {
	...
	private void doClose() {
		synchronized (readLock) {
			if (fd != null) {
				try {
					//System.out.println("Time:"+java.time.LocalTime.ofNanoOfDay(java.lang.System.currentTimeMillis() % 86_400_000 * 1_000_000L).format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss,SSS"))+"; Method: Pack.doClose(); packed file  "+this.getPackName());
					fd.close();
