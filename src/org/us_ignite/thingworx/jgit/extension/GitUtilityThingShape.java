...
public class GitUtilityThingShape extends Thing {
    ...
    public InfoTable RepositoryCreate(
        ...
        try {
            ...
            repoThing.setPropertyValue(
                    Const.str_GitRepoURL,
                    new StringPrimitive(GitRepoURL == null ? "" : GitRepoURL));
            repoThing.setPropertyValue(
            ...
            repoThing.setPropertyValue(
                    Const.str_ProjectName,
                    new StringPrimitive(ProjectName == null ? "" : ProjectName));
            repoThing.setPropertyValue(
            ...
            return ServiceResults.fromError(
                    "RepositoryCreate", e, "RepositoryCreate Error: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            if (searcher == null)
                return ServiceResults.payload(
                        "GIT.RepositoryList.ServiceResult.DataShape", result, "");
            ...
            return ServiceResults.payloadFailure(
                    "GIT.RepositoryList.ServiceResult.DataShape",
                    "RepositoryList Error: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            try {
                ...
                str_RepositoryPathName =
                        raw == null || isBlank(raw.toString()) ? "" : raw.toString();
            } catch (Exception e) {
                str_RepositoryPathName = "";
            }
            ...
            try {
                ...
                _logger.error(
                        "Deleting Git Repository folder failed when deleting Thing " + RepoName);
            }
            ...
            return ServiceResults.fromError(
                    "RepositoryDelete", e, "RepositoryDelete Error: " + e.getMessage());
        }
    }
    ...
    public InfoTable InitUserExtensionProperties() {
        try {
            ...
            if (user != null) {
                try {
                    hasGitCredentials =
                            user.getPropertyValue("UserRepositoryConfiguration") != null;
                } catch (Exception e) {
                }
            }
                        ...
                        if (gpgKeys instanceof InfoTablePrimitive) {
                            ...
                            if (table != null) {
                                ...
                                for (int i = 0; i < table.getRowCount(); i++) {
                                    ...
                                    String fingerprint =
                                            primitiveString(source, Const.str_GpgKeyFingerprint);
                                    if (!isBlank(fingerprint)) {
                                        ValueCollection key = new ValueCollection();
                                        key.put(
                                                Const.str_GpgKeyFingerprint,
                                    ...
                                    for (int j = 0; j < empty.getRowCount(); j++) {
                                        if (gitThing.equals(
                                                primitiveString(empty.getRow(j), "GitThing"))) {
                                ...
                                user.setPropertyValue(
                                        Const.str_UserGpgKeys, new InfoTablePrimitive(keyStore));
                            }
                        }
                        user.setPropertyValue(
            ...
            if (!hasUserGpgKeys) {
                ...
                addPropParams.put("persistent", new BooleanPrimitive(true));
                addPropParams.put(
                        "dataShape", new StringPrimitive(Const.str_UserGpgKeyDataShapeName));
                userExtensions.processServiceRequest("AddPropertyDefinition", addPropParams);
                new EntityServices().RestartDependenciesForThingShape("UserExtensions");
            }
                ...
                for (String propertyName :
                    ...
                    for (int i = 0; i < source.getRowCount(); i++) {
                        ...
                        if (row.getPrimitive(Const.str_GpgPrivateKey) != null)
                            key.put(
                                    Const.str_GpgPrivateKey,
                                    row.getPrimitive(Const.str_GpgPrivateKey));
            ...
            return ServiceResults.fromError(
                    ...
                    e,
                    "InitUserExtensionProperties Error: " + e.getMessage());
        }
    }
    ...
    public InfoTable VerifyGpgKey(
        ...
        InfoTable result =
                ServiceResults.emptyPayload(Const.str_GpgKeyVerificationResultDataShapeName);
        try {
    ...
    public InfoTable GpgKeyList() {
        try {
            ...
            if (gpgKeys == null)
                gpgKeys = ServiceResults.emptyPayload(Const.str_UserGpgKeyDataShapeName);
    ...
    public InfoTable GpgKeyCreate(
        ...
        try {
            ...
            return ServiceResults.fromError(
                    "GpgKeyCreate", e, "GpgKeyCreate Error: " + e.getMessage());
        }
    }
    ...
    public InfoTable GpgKeyGet(
        ...
        try {
            ...
            return ServiceResults.fromError(
                    "GIT.GpgKey.ServiceResult.DataShape",
                    "GpgKeyGet",
                    e,
                    "GpgKeyGet Error: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < keys.getRowCount(); i++)
                if (fingerprint != null
                    ...
                    keys.getRow(i)
                            .put(Const.str_GpgKeyPassphrase, new PasswordPrimitive(passphrase));
                    found = true;
                }
            ...
            return ServiceResults.fromError(
                    "GpgKeyUpdate", e, "GpgKeyUpdate Error: " + e.getMessage());
        }
    }
    ...
    public InfoTable GpgKeyDelete(
        ...
        try {
            ...
            if (keys != null) {
                ...
                for (int i = 0; i < keys.getRowCount(); i++) {
                    ...
                    if (GpgKeyFingerprint.equals(
                            primitiveString(row, Const.str_GpgKeyFingerprint))) {
                        found = true;
                        continue;
                    }
                    cloned.addRow(row);
            ...
            if (!found)
                throw new IllegalArgumentException("GPG key not found: " + GpgKeyFingerprint);
            InfoTable configurations = getGitCredentials(currentUser);
            if (configurations != null)
                for (int i = 0; i < configurations.getRowCount(); i++)
                    if (GpgKeyFingerprint.equals(
                            primitiveString(
                                    configurations.getRow(i), Const.str_GpgKeyFingerprint))) {
                        configurations
                                .getRow(i)
    ...
    public InfoTable GitCredentialList() {
        try {
            ...
            if (result == null)
                result = ServiceResults.emptyPayload(Const.str_GitCredentialsDataShapeName);
            return ServiceResults.payload(
        ...
        try {
            ...
            if (all != null)
                for (int i = 0; i < all.getRowCount(); i++)
                    if (gitThing != null
                            && gitThing.equals(primitiveString(all.getRow(i), "GitThing")))
            ...
            for (int i = 0; i < all.getRowCount(); i++)
                if (gitThing != null
                    ...
                    if (hasText(GpgKeyFingerprint))
                        row.put(
                                Const.str_GpgKeyFingerprint,
                                new StringPrimitive(GpgKeyFingerprint));
    ...
    public InfoTable GitCredentialDelete(
        ...
        try {
            ...
            for (int i = 0; i < all.getRowCount(); i++) {
                ValueCollection row = all.getRow(i);
                if (gitThing != null && gitThing.equals(primitiveString(row, "GitThing")))
                    found = true;
