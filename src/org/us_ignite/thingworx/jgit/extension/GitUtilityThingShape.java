...
public class GitUtilityThingShape extends Thing {
    ...
    public InfoTable RepositoryCreate(
            ...
            @ThingworxServiceParameter(name = "RepoPathName", description = "", baseType = "STRING")
                    String RepoPathName,
            @ThingworxServiceParameter(name = "BranchName", description = "", baseType = "STRING") String BranchName,
            ...
            @ThingworxServiceParameter(name = "LocalizationTokensPrefix", description = "", baseType = "STRING")
                    String LocalizationTokensPrefix,
            @ThingworxServiceParameter(name = "GitCommitterUser", description = "", baseType = "STRING")
                    String GitCommitterUser,
            @ThingworxServiceParameter(name = "GitCommitterPassword", description = "", baseType = "STRING")
                    String GitCommitterPassword,
            @ThingworxServiceParameter(name = "GitCommitterEmail", description = "", baseType = "STRING")
                    String GitCommitterEmail,
            @ThingworxServiceParameter(name = "GitCommitterFullName", description = "", baseType = "STRING")
        ...
        try {
            ...
            if (EntityUtilities.findEntity(repositoryProject, ThingworxRelationshipTypes.Project) == null) {
                es.CreateProject(
                        repositoryProject,
                        "Component",
            ...
            if (ProjectName != null
                    && !ProjectName.isBlank()
                    && EntityUtilities.findEntity(ProjectName, ThingworxRelationshipTypes.Project) == null) {
                es.CreateProject(
                        ProjectName,
                        "Component",
            ...
            es.CreateThing(
                    RepoName,
                    "GitRepository created by user " + UserUtilities.getCurrentUser() + " at " + new java.util.Date(),
                    new TagCollection(),
                    repositoryProject,
                    "FileRepository");
            ...
            repoThing.setPropertyValue(Const.GitRepoURL, new StringPrimitive(GitRepoURL == null ? "" : GitRepoURL));
            repoThing.setPropertyValue(
                    Const.RepoPathName, new StringPrimitive(RepoPathName == null ? "" : RepoPathName));
            repoThing.setPropertyValue(
                    Const.InitialBranch, new StringPrimitive(BranchName == null ? "main" : BranchName));
            repoThing.setPropertyValue(Const.ProjectName, new StringPrimitive(ProjectName == null ? "" : ProjectName));
            repoThing.setPropertyValue(Const.UseProxy, new BooleanPrimitive(UseProxy != null && UseProxy));
            repoThing.setPropertyValue(Const.ProxyURL, new StringPrimitive(ProxyURL == null ? "" : ProxyURL));
            ...
            repoThing.setPropertyValue(
                    ...
                    new StringPrimitive(LocalizationTokensPrefix == null ? "" : LocalizationTokensPrefix));
            GitCredentialCreate(
                    GitCommitterUser, GitCommitterPassword, GitCommitterEmail, GitCommitterFullName, RepoName, "");
    ...
    public InfoTable RepositoryList() {
        try {
            InfoTable result = InfoTableInstanceFactory.createInfoTableFromDataShape("GIT.RepositoryList.DataShape");
            Searcher searcher =
                    (Searcher) EntityUtilities.findEntity("SearchFunctions", ThingworxRelationshipTypes.Resource);
            if (searcher == null)
                return ServiceResults.successFromPayload(
                        "RepositoryList", "GIT.RepositoryList.ServiceResult.DataShape", result);
            JSONObject empty = new JSONObject();
            InfoTable found = searcher.SpotlightSearch(
                    "",
                    new TagCollection(),
                    empty,
                    empty,
            ...
            for (int i = 0; i < found.getRowCount(); i++) {
                ...
                out.put(
                        "RepoName",
                        new StringPrimitive(Optional.ofNullable(row)
            ...
            return ServiceResults.fromError("RepositoryList", e, "GIT.RepositoryList.ServiceResult.DataShape");
        }
    }

    @ThingworxServiceDefinition(
            name = "RepositoryDelete",
            description = "Deletes a GIT Repository Thing and its local FileRepository working tree.",
            category = "",
            isAllowOverride = false,
            aspects = {"isAsync:false"})
    @ThingworxServiceResult(
    ...
    public InfoTable RepositoryDelete(
        ...
        try {
            ...
            if (user != null) {
                ...
                if (creds != null) {
                    InfoTable updatedCreds = InfoTableInstanceFactory.createInfoTableFromDataShape(
            ...
            _logger.warn(
                    "GIT Repository Thing " + RepoName + " (self-hosted FileRepository) was deleted successfully.");
    ...
    public InfoTable InitUserExtensionProperties() {
        try {
            ThingShape userExtensions =
                    (ThingShape) EntityUtilities.findEntity("UserExtensions", ThingworxRelationshipTypes.ThingShape);
            ...
            if (!hasGitCredentials) {
                ...
                addPropParams.put(
                        "dataShape", new StringPrimitive("GIT.RepositoryConfiguration.UserExtension.DataShape"));
            ...
            if (user != null) {
                ...
                if (configuration == null) {
                    InfoTable empty =
                            InfoTableInstanceFactory.createInfoTableFromDataShape(Const.GitCredentialsDataShapeName);
                    ...
                    if (gpgKeys instanceof InfoTablePrimitive) {
                        ...
                        if (table != null) {
                            InfoTable keyStore = InfoTableInstanceFactory.createInfoTableFromDataShape(
                                    Const.UserGpgKeyDataShapeName);
                            for (int i = 0; i < table.getRowCount(); i++) {
                                ValueCollection source = table.getRow(i);
                                String gitThing = Optional.ofNullable(source)
                                        .map(r -> r.getPrimitive("GitThing"))
                                        .map(IPrimitiveType::getValue)
                                        .map(Object::toString)
                                        .orElse(null);
                                String fingerprint = Optional.ofNullable(source)
                                ...
                                if (fingerprint != null && !fingerprint.isBlank()) {
                                    ...
                                    if (source.getPrimitive(Const.GpgPrivateKey) != null)
                                        key.put(Const.GpgPrivateKey, source.getPrimitive(Const.GpgPrivateKey));
                                    if (source.getPrimitive(Const.GpgKeyPassphrase) != null)
                                        key.put(Const.GpgKeyPassphrase, source.getPrimitive(Const.GpgKeyPassphrase));
                                    keyStore.addRow(key);
                                }
                                ValueCollection target = null;
                                for (int j = 0; j < empty.getRowCount(); j++) {
                                    if (gitThing.equals(Optional.ofNullable(empty.getRow(j))
                                            .map(r -> r.getPrimitive("GitThing"))
                                            .map(IPrimitiveType::getValue)
                                            .map(Object::toString)
                                            .orElse(null))) {
                                ...
                                if (fingerprint != null && !fingerprint.isBlank())
                                    target.put(Const.GpgKeyFingerprint, new StringPrimitive(fingerprint));
                            }
                            user.setPropertyValue(Const.UserGpgKeys, new InfoTablePrimitive(keyStore));
                        }
                    }
                ...
                String[] oldProps = {
                    "UseGitCommitUserValues", "GitCommitterEmail", "GitCommitterPassword", "GitCommitterName"
                };
            ...
            if (user != null && user.getPropertyValue(Const.UserGpgKeys) == null) {
                InfoTable keys = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.UserGpgKeyDataShapeName);
                for (String propertyName : new String[] {"GpgKeys", "UserRepositoryConfiguration"}) {
                    ...
                    for (int i = 0; i < source.getRowCount(); i++) {
                        ...
                        for (int j = 0; j < keys.getRowCount(); j++) {
                            if (fingerprint.equals(keys.getRow(j).getStringValue(Const.GpgKeyFingerprint))) {
                                alreadyStored = true;
                                break;
                            }
                        }
                        ...
                        if (row.getPrimitive(Const.GpgKeyPassphrase) != null)
                            key.put(Const.GpgKeyPassphrase, row.getPrimitive(Const.GpgKeyPassphrase));
                        keys.addRow(key);
                    }
                }
    ...
    @ThingworxServiceResult(
            ...
            description = "",
            baseType = "INFOTABLE",
            aspects = {"isEntityDataShape:true", "dataShape:GIT.GpgKeyVerification.ServiceResult.DataShape"})
    public InfoTable VerifyGpgKey(
        ...
        try {
            result = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.GpgKeyVerificationResultDataShapeName);
            ...
            if (key.isBlank()) {
                ...
                ValueCollection stored =
                        storedKeys == null || storedKeys.getRowCount() == 0 ? null : storedKeys.getRow(0);
                if (stored != null) {
                    key = ((PasswordPrimitive) stored.getPrimitive(Const.GpgPrivateKey)).getValue();
                    passphrase = ((PasswordPrimitive) stored.getPrimitive(Const.GpgKeyPassphrase)).getValue();
                }
            }
            ...
            return ServiceResults.fromError("VerifyGpgKey", e, "GIT.GpgKeyVerification.ServiceResult.DataShape");
        }
        return ServiceResults.successFromPayload(
    ...
    @ThingworxServiceDefinition(
            name = "GpgKeyList",
            description = "Returns all GPG keys configured for the current user across all repositories",
            category = "",
            isAllowOverride = false,
            aspects = {"isAsync:false"})
    @ThingworxServiceResult(
    ...
    public InfoTable GpgKeyList() {
        try {
            ...
            if (gpgKeys == null)
                gpgKeys = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.UserGpgKeyDataShapeName);
            return ServiceResults.successFromPayload("GpgKeyList", "GIT.GpgKey.ServiceResult.DataShape", gpgKeys);
        } catch (Exception e) {
        ...
        try {
            ...
            if (GpgKeyFingerprint.isBlank()) {
                if (GpgPrivateKey.isBlank())
                    throw new IllegalArgumentException("GpgPrivateKey or GpgKeyFingerprint is required.");
            ...
            if (gpgKeys == null)
                ...
                InfoTable cloned = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.UserGpgKeyDataShapeName);
                for (int i = 0; i < gpgKeys.getRowCount(); i++) {
                    ...
                    if (GpgKeyFingerprint.equals(row.getStringValue(Const.GpgKeyFingerprint)))
                        throw new IllegalArgumentException("A GPG key with this fingerprint already exists.");
                    cloned.addRow(row);
                }
                gpgKeys = cloned;
            }
    ...
    public InfoTable GpgKeyGet(
            @ThingworxServiceParameter(name = "GpgKeyFingerprint", description = "Key fingerprint", baseType = "STRING")
                    String fingerprint) {
        try {
            User user = new GitUserContextManager().requireUser();
            InfoTable result = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.UserGpgKeyDataShapeName);
            InfoTable keys = getGpgKeysTable(user);
            if (keys != null)
                for (int i = 0; i < keys.getRowCount(); i++)
                    if (fingerprint != null
                            && fingerprint.equals(keys.getRow(i).getStringValue(Const.GpgKeyFingerprint)))
                        result.addRow(keys.getRow(i));
            if (result.getRowCount() == 0) throw new IllegalArgumentException("GPG key not found: " + fingerprint);
            return ServiceResults.successFromPayload("GpgKeyGet", "GIT.GpgKey.ServiceResult.DataShape", result);
        } catch (Exception e) {
    ...
    public InfoTable GpgKeyUpdate(
        ...
        try {
            ...
            boolean found = false;
            for (int i = 0; i < keys.getRowCount(); i++)
                if (fingerprint != null && fingerprint.equals(keys.getRow(i).getStringValue(Const.GpgKeyFingerprint))) {
                    keys.getRow(i).put(Const.GpgPrivateKey, new PasswordPrimitive(privateKey));
                    keys.getRow(i).put(Const.GpgKeyPassphrase, new PasswordPrimitive(passphrase));
                    found = true;
                }
        ...
        try {
            ...
            if (configurations != null)
                for (int i = 0; i < configurations.getRowCount(); i++)
                    if (GpgKeyFingerprint.equals(configurations.getRow(i).getStringValue(Const.GpgKeyFingerprint))) {
                        configurations.getRow(i).put(Const.GpgKeyFingerprint, new StringPrimitive(""));
                    }
            if (configurations != null)
                currentUser.setPropertyValue(Const.UserRepositoryConfiguration, new InfoTablePrimitive(configurations));
            return ServiceResults.successFromString("GpgKeyDelete", "GPG key deleted.");
        } catch (Exception e) {
            return ServiceResults.fromError("GpgKeyDelete", e);
        }
    }
    ...
    public InfoTable GitCredentialCreate(
            ...
            @ThingworxServiceParameter(
                            name = "GpgKeyFingerprint",
                            description = "Optional GPG key fingerprint; when set, commits are signed with this key",
                            baseType = "STRING")
                    String GpgKeyFingerprint) {
        try {
            InitUserExtensionProperties();
            ...
            InfoTable creds = new GitUserContextManager().credentials();
            if (creds == null) {
                creds = InfoTableInstanceFactory.createInfoTableFromDataShape(
                        "GIT.RepositoryConfiguration.UserExtension.DataShape");
            } else {
                InfoTable cloned = InfoTableInstanceFactory.createInfoTableFromDataShape(
                        "GIT.RepositoryConfiguration.UserExtension.DataShape");
                for (int i = 0; i < creds.getRowCount(); i++) {
                    ...
                    if (row.getPrimitive("GitThing").getValue().toString().equals(GitThing))
                        throw new IllegalArgumentException("Credentials already exist for repository: " + GitThing);
                    cloned.addRow(row);
                }
                creds = cloned;
            }
            ...
            return ServiceResults.successFromString("GitCredentialCreate", "Git credentials created.");
        } catch (Exception e) {
    ...
    @ThingworxServiceResult(
            ...
            description = "",
            baseType = "INFOTABLE",
            aspects = {"isEntityDataShape:true", "dataShape:GIT.RepositoryConfiguration.ServiceResult.DataShape"})
    public InfoTable GitCredentialList() {
        try {
            InfoTable result = new GitUserContextManager().credentials();
            if (result == null)
                result = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.GitCredentialsDataShapeName);
            return ServiceResults.successFromPayload(
                    "GitCredentialList", "GIT.RepositoryConfiguration.ServiceResult.DataShape", result);
        } catch (Exception e) {
    ...
    public InfoTable GitCredentialGet(
            @ThingworxServiceParameter(name = "GitThing", description = "Repository Thing", baseType = "THINGNAME")
                    String gitThing) {
        try {
            InfoTable result = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.GitCredentialsDataShapeName);
            ...
            return ServiceResults.successFromPayload(
                    "GitCredentialGet", "GIT.RepositoryConfiguration.ServiceResult.DataShape", result);
        } catch (Exception e) {
            return ServiceResults.fromError(
    ...
    public InfoTable GitCredentialUpdate(
            @ThingworxServiceParameter(name = "GitCommitterUser", description = "", baseType = "STRING") String user,
            @ThingworxServiceParameter(name = "GitCommitterPassword", description = "", baseType = "STRING")
                    String password,
            @ThingworxServiceParameter(name = "GitCommitterEmail", description = "", baseType = "STRING") String email,
            @ThingworxServiceParameter(name = "GitCommitterFullName", description = "", baseType = "STRING")
                    String fullName,
            @ThingworxServiceParameter(name = "GitThing", description = "", baseType = "THINGNAME") String gitThing,
            @ThingworxServiceParameter(
        ...
        try {
            ...
            InfoTable all = new GitUserContextManager().credentials();
            if (all == null) throw new IllegalArgumentException("Credentials not found for repository: " + gitThing);
            boolean found = false;
            for (int i = 0; i < all.getRowCount(); i++)
                if (gitThing != null && gitThing.equals(all.getRow(i).getStringValue("GitThing"))) {
            ...
            if (!found) throw new IllegalArgumentException("Credentials not found for repository: " + gitThing);
            current.setPropertyValue(Const.UserRepositoryConfiguration, new InfoTablePrimitive(all));
            return ServiceResults.successFromString("GitCredentialUpdate", "Git credentials updated.");
        } catch (Exception e) {
    ...
    public InfoTable GitCredentialDelete(
        ...
        try {
            ...
            InfoTable all = new GitUserContextManager().credentials();
            if (all == null) throw new IllegalArgumentException("Credentials not found for repository: " + gitThing);
            InfoTable kept = InfoTableInstanceFactory.createInfoTableFromDataShape(Const.GitCredentialsDataShapeName);
            boolean found = false;
            for (int i = 0; i < all.getRowCount(); i++) {
                ValueCollection row = all.getRow(i);
                if (gitThing != null && gitThing.equals(row.getStringValue("GitThing"))) found = true;
                else kept.addRow(row);
            }
            if (!found) throw new IllegalArgumentException("Credentials not found for repository: " + gitThing);
            current.setPropertyValue(Const.UserRepositoryConfiguration, new InfoTablePrimitive(kept));
            return ServiceResults.successFromString("GitCredentialDelete", "Git credentials deleted.");
        } catch (Exception e) {
