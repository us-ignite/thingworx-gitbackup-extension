...
public class ThingWorxTestStack implements AutoCloseable {
    ...
    protected ThingWorxTestStack(
        ...
        if (needsSecurity) {
            ...
            try {
                new ProcessBuilder("chmod", "777", sharedStorage.toAbsolutePath().toString()).start().waitFor();
            ...
            securityInit =
                    new org.testcontainers.containers.GenericContainer<>(securityImage)
                            ...
                            .withEnv("CUSTOM_SECRET_LIST", "encrypt.db.password:TWX_DATABASE_PASSWORD")
                            .withEnv("TWX_DATABASE_PASSWORD", credentials.twxDatabasePass)
                            .withEnv("SECRET_PROVISIONING_APP_KEY", "test-provisioning-key-12345")
                            .withFileSystemBind(
                                    sharedStorage.toAbsolutePath().toString(),
                                    "/ThingworxStorage",
                                    org.testcontainers.containers.BindMode.READ_WRITE)
                            .waitingFor(
                                    org.testcontainers.containers.wait.strategy.Wait.forLogMessage(".*encrypt.db.password stored.*", 1)
