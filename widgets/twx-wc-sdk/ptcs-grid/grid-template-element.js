...
const gridDisabledField = Symbol('grid-disabled');
...
// Grid has changed disabled state. Inform children
function performGridDisable(disabled) {
    this[gridDisabledField] = disabled;
    const _disabled = disabled || this[disabledField];
    setAttribute(this, 'disabled', _disabled);

    for (const child of this.children) {
        if (typeof child.performGridDisable === 'function') {
            child.performGridDisable(disabled);
        } else {
            setAttribute(child, 'disabled', _disabled);
        }
    }
}

// Compute value of disabled attribute for this element. Own state is disabled, grid state is this[gridDisabledField]
function disableAttr(disabled) {
    this[disabledField] = !!disabled;
    return disabled || this[gridDisabledField];
}
...
function processElem(el, opt) {
    ...
    for (let i = 0; i < attributes.length; i++) {
        ...
        if (name === 'disabled') {
            // Special handling of disabled state, which depends on both the grids state and the items state
            exprAttr.disabled = typeof text === 'function' ? (item, e) => disableAttr.call(e, text(item)) : (_, e) => disableAttr.call(e, text);
        } else {
            switch (typeof text) {
                case 'string':
                    constAttr[name] = text;
    ...
    // grid-action?
    if (constAttr.hasOwnProperty('grid-action')) {
        constAttr.tabindex = '-1';
    }
...
function generateGridEvent(event) {
    const {target} = event;
    const evName = target[eventField] && target[eventField][event.type];
    if (!evName) {
    ...
    let grid;
    let row;
    for (let el = target; el && !grid; el = el.assignedSlot || (el.nodeType === 11 && el.host) || el.parentNode) {
        if (el.nodeType !== 1) {
            continue;
        }
        if (el.part.contains('row') && typeof el.index === 'number' && !row) {
            row = el;
        }
        if (el.tagName === 'PTCS-GRID') {
            grid = el;
        }
    }
    if (!grid) {
...
function createElem(ctor, a) {
    ...
    el.performGridDisable = performGridDisable;

    // Fixed attributes
    if (ctor.constAttr) {
        for (const name in ctor.constAttr) {
            setAttribute(el, name, ctor.constAttr[name]);
        }
    }
    ...
    if (exprAttr) {
        a.push(item => {
            for (const name in exprAttr) {
                setAttribute(el, name, exprAttr[name](item, el));
            }
        });
    }
...
// If el is a template, extract and return clone of first child element, otherwise return el
function _resolve(el) {
    return el.tagName === 'TEMPLATE' && el.content.children[0] ? el.content.children[0].cloneNode(true) : el;
}


// {create(), assign(item)}
export function createTemplateElement(el, opt) {
    const ctor = processElem(_resolve(el), Object.assign({prefix, suffix}, opt || {}));
    return {
        create: () => {
            const a = [];
...
export function createTemplateHeader(el) {
    const ctor = processElem(_resolve(el), {prefix: null, suffix: null});
