...
|sort-order |sortOrder |String: specify optional initial sort order, 'asc' (ascending) or 'desc' (descending) or unspecified (no sorting)|
...
|resolve |resolve |Boolean: Resolve value? |
...
## Replace template compiler

The grid has a default template compiler. The compiler supports:

- interpolation (see above)
- value modifiers (see above)
- grid-event-map translation (see above)
- strip attributes (see above)
- assignment of disabled attributes (both the grid and the item may affect the disabled attribute)
- propagates grid disabled events to sub-elements
- automatically sets tabindex="-1" for grid-action elements

The compiler implementation is in `src\components\ptcs-grid\grid-template-element.js`.

The compiler handles intrinsic html element and web components. It does not support framework specific templates, such that are found in React, Angular, Aurelia, etc. If there is a requirement to support such templates, the template compiler can be replaced.

Note that if you choose to replace the template compiler, you will no longer have the built-in support for the features of the native template compiler (interpolation, value modifiers, grid-event-map, etc).

This compiler is replaced using the `templateCompiler` property. The templateController should be assigned to an object with one or two functions:

~~~js
grid.templateController = {
    compileHead: (templateEl) => {...}
    compileCell: (templateEl, options) => {...}
};
~~~

If a function is missing, the grid fallbacks to the default compiler function. The default functions correspond to (`src\components\ptcs-grid\grid-template-element.js`):

- `compileHead`: `createTemplateHeader`
- `compileCell`: `createTemplateElement`


`compileHead` will be invoked with a template element and should return a function  that creates a header element.


`compileCell` will be invoked with a template element and should return an `$uiCtrl` object. (See `src\components\ptcs-grid\grid-view.md` for details). `compileCell` may be called with an optional options argument that (currently) only specifies the interpolation values.
...
<ptcs-grid>
    <ptcs-grid-column-def>
        <div grid-header><em>Name</em></div>
        <div>Name: ${name}</div>
    </ptcs-grid-column-def>
    <ptcs-grid-column-def>
        <div grid-header><em>Age</em></div>
        <div>Age: ${age}</div>
    </ptcs-grid-column-def>
</ptcs-grid>
...
`compileHead` will be called with `<div grid-header><em>Name</em></div>` and `<div grid-header><em>Age</em></div>`. `compileCell` will be invoked with `<div>Name: ${name}</div>` and `<div>Age: ${age}</div>`.
