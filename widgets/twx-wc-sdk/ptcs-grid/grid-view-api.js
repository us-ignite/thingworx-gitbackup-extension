...
const prop2attr = {
    ...
    maxWidth:     'max-width',     // The maximum column width when the width property contains is a dynamic value
    sortable:     'sortable',      // A Boolean property that displays a sort icon in the column header
    sortOrder:    'sort-order',    // A String property that specifies the inital sort order, 'asc' (ascending) or 'desc' (descending)
    ...
    resolve:      'resolve',       // Resolve value via Value Manager?
...
export class DataViewerAPI extends DataViewer {
    ...
    constructor(gridEl, columnDefName) {
        super();
    ...
    mutationEvent(mutations) {
        const name = `${this._columnDefName},${this._gridEl.tagName}`;
        const isColDef = el => (el instanceof Element ? el : el.parentElement).closest(name) !== this._gridEl;

        // Ignore changes on non-column-defining elements
        if (mutations.some(mutation => isColDef(mutation.target))) {
            this.rebuildColumnDefs();
        }
    }
    ...
    set templateCompiler(_templateCompiler) {
        if (this._templateCompiler !== _templateCompiler) {
    ...
    _decodeColumn(coldef) {
        ...
        if (coldef.firstElementChild) {
            const tc = this._templateCompiler || {};
            let headerEl = coldef.querySelector(':scope > [grid-header]');
            let cellTmpl = coldef.querySelector(':scope > :not([grid-header])');

            // Maybe both header and cell template is inside the cell template?
            if (!headerEl) {
                if (cellTmpl && cellTmpl.tagName === 'TEMPLATE' && cellTmpl.content.children.length > 1) {
                    // header and cell template might reside inside the same template
                    headerEl = [...cellTmpl.content.children].find(el => el.hasAttribute('grid-header'))?.cloneNode(true);
                    if (headerEl) {
                        cellTmpl = [...cellTmpl.content.children].find(el => !el.hasAttribute('grid-header'))?.cloneNode(true);
                    }
                } else if (cellTmpl && cellTmpl.querySelector('[grid-header]') && cellTmpl.querySelector('[grid-cell]')) {
                    // Special fix for Aurelia v1.3, which forcefully wraps the column elements in an au-content element
                    headerEl = cellTmpl.querySelector('[grid-header]');
                }
            }

            if (headerEl) {
                // Assign title to label, if name is unassigned. Otherwise the label will be lost, and sorting becomes impossible.
                if (column.title === undefined && column.label !== undefined) {
                    column.title = column.label; // E.g. this makes it possible to sort the column
                }
                column.label = (tc.compileHead || createTemplateHeader)(headerEl);
            }
            if (cellTmpl) {
                // Special fix for Aurelia v1.3, which forcefully wraps the column elements in an au-content element
                column.$uiCtrl = (tc.compileCell || createTemplateElement)(cellTmpl.querySelector('[grid-cell]') || cellTmpl, this._interpolation);
            }
        }
