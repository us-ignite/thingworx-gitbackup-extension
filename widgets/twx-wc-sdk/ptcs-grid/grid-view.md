...
- `sortOrder` - The initial sort order, 'asc' (ascending) or 'desc' (descending). Ignored unless `sortable` is enabled.
- `editable` - A Boolean property that controls whether inline editing is enabled or disabled for the column cells.
- `resolve` - A Boolean property that controls whether the cell value should be resolved via the `Value Manager`.
...
### Using the resolve Field

The optional `resolve` field is a Boolean that marks the cell value as _delayed_ if `true`.

A delayed value is resolved via the _Value Manager_ (see separate document). With this feature enabled, the grid can show rows before they have been fully retrieved. The missing value is typically indicated with some kind of "spinner". The delayed values are resolved when the client has the values. The client can resolve values in any order, but the Value Manger helps by informing about the values that are currently visible in the grids viewport.

When using the `resolve` feature, the `value` field must be a string. That string will be used as the _column-id_ of the value in the Value Manager. If the grid data has non-undefined values with the same field name, the Value Manager will consider those values to already have been resolved.
