...
export class DataManager {
    ...
    constructor(items) {
        ...
        this._sortOrg = null; // Sort function given by client
        this._defaultSort = null; // Default sort function
        this._sort = null; // Actual sort function (depends on _sortOrg and _defaultSort)
    ...
    // Notify observers
    _msg(msg, ...arg) {
        const old = this._messing;
        console.assert(old === undefined); // _msg() should not be called recursively, but if that happens, don't crash, just warn...
    ...
    // Some item values has changed, so view might need to be regenerated.
    valuesChanged() {
        if (this._sort || this._filter) {
            const _oldView = this._view;
            this._rebuildView();

            // Only use dmView if the view actually changed
            if (PTCS.sameArray(_oldView, this._view)) {
                return false;
            }
            this._msg('dmView');
            return true;
    ...
    updateBaseItem(baseIndex, index, update, validation) {
        if (!(0 <= baseIndex && baseIndex < this._items.length)) {
            throw new Error(`Invalid index: ${baseIndex}`);
        }
    ...
    updateItem(index, update, validation) {
        this.updateBaseItem(this.baseIndex(index), index, update, validation);
    }
    ...
    **********************************************************************/
    ...
    // Note: isDisabled is currently only used when auto-selecting child nodes in tree data
    set isDisabled(_isDisabled /* : baseIndex => true || false*/) {
        this._isDisabled = typeof _isDisabled === 'function' ? _isDisabled : undefined;
    }
    ...
    set defaultSort(defaultSort) {
        console.assert(!defaultSort || typeof defaultSort === 'function');

        if (this._defaultSort !== (defaultSort || null)) {
            this._defaultSort = (defaultSort || null);
            this._assignSort();
        }
    }
    ...
    get sort() {
        return this._sortOrg; // Return the sort function that the client assigned
    }

    // Set sort function
    set sort(sort) {
        console.assert(!sort || typeof sort === 'function');

        if (this._sortOrg !== (sort || null)) {
            this._sortOrg = (sort || null);
            this._assignSort();
        }
    }

    _assignSort() {
        const _sort1 = this._sortOrg;
        const _sort2 = this._defaultSort;
        ...
        const createSortFunction = () => {
            ...
            if (_sort1) {
                if (_sort2) {
                    ...
                    return (i1, i2) => _sort1(this._items[i1], this._items[i2], i1, i2, this) || _sort2(this._items[i1], this._items[i2], i1, i2, this) || i1 - i2;
                }
                return (i1, i2) => _sort1(this._items[i1], this._items[i2], i1, i2, this) || i1 - i2;
            }
            if (_sort2) {
                return (i1, i2) => _sort2(this._items[i1], this._items[i2], i1, i2, this) || i1 - i2;
            }
            return null;
        };

        this._sort = createSortFunction();
    ...
    get expandedOrCollapsed() {
        const r = this._expandOrCollapse;
        this._expandOrCollapse = undefined;
        return r;
    }
    ...
    isCollapsedNode(baseIndex) {
        const item = this._items[baseIndex];
        if (!item || !this._subItemsState) {
            return false;
        }
        const map = this._mapSubItems?.get(item);
        const state = map ? map.isOpen : this._subItemsState(item, baseIndex);
        // Is the children already loaded but collapsed or is the node marked has having (retrievable) children?
    ...
    subTree(index, state) {
        ...
        } else if (this.childDataServiceEvent) {
            this._subItems(item, baseIndex);
        }
    ...
    expandSome(filter) {
        ...
        const expand = (baseIndex, level) => {
            ...
            if (filter(item)) {
                if (map) {
                    ...
                    if (!map.isOpen) {
                        // Children is already mounted, but not expanded. Expand them.
                        map.isOpen = true;
                        this._expandOrCollapse = true;
                    }
                    ...
                    const sub = this._subItems(item, baseIndex);
                    if (sub) {
                        map = this._registerSubNodes(baseIndex, sub, true, 1 + level, ixs);
                    }
                }
            }
            if (map?.isOpen) {
                for (let bi = map.start; bi < map.end; bi++) {
                    expand(bi, 1 + level);
                }
            }
        };
        ...
        if (this._view) {
            this._view.forEach(baseIndex => expand(baseIndex, this.baseLevel(baseIndex)));
        }
    ...
    */
    ...
    _selectSubRange(start, end, ixs) {
        ...
        for (let i = start; i < end; i++) {
            ...
            if (!this._selectedSet.has(item) && !(this._isDisabled && this._isDisabled(i))) {
    ...
    _multiUnselect(baseIndex, ix, directSelection) {
        ...
        if (ixs.length > 1) {
            ...
            const selectedValue = a.length > 0 ? a : null;
            ...
            this._msg('dmSelected', baseIndex, false);
            this._msg('dmSelection', this._selected);
        }
    }
    ...
    select(baseIndex, select, directSelection) {
        if (this.selectMethod === 'single') {
            ...
            if (select === true) {
                ...
                const selectedValue = this._selected !== baseIndex ? baseIndex : null;
                this.setSelected(selectedValue, directSelection);
            }
        } else if (this.selectMethod === 'multiple') {
    ...
    selectAllItems(directSelection) {
        if (this._view) {
            const ixs = [];
            for (const baseIndex of this._view) {
                ixs.push(baseIndex);
                this._enumerateLoadedDescendants(baseIndex, (from, to) => {
                    for (let i = from; i < to; i++) {
                        ixs.push(i);
    ...
    submitIfValid(baseIndex, field) {
        ...
        if (field) {
            ...
            for (const f in updated) {
                if (f !== '$validation' && item[f] !== updated[f]) {
    ...
    getEditedItems(level) {
        ...
        const hasValidChange = update => {
            for (const field in update) {
                if (field !== '$validation' && !update.$validation[field]) {
                    return true;
                }
            }
            return false;
        ...
        const hasInvalidChange = update => {
            // eslint-disable-next-line no-unused-vars
            for (const field in update.$validation) {
        ...
        const createValidItem = (update, item) => {
            let _item;
            for (const _field in update.$validation) {
                if (!_item) {
                    _item = Object.assign({}, item); // Need to restore at least one value. Duplicate item
                }
        ...
        if (level === 'cell') {
            this._updatedItems.forEach((update, item) => hasValidChange(update) && a.push(createValidItem(update, item)));
        } else {
            // Collect all items that are fully valid
            this._updatedItems.forEach((update, item) => !hasInvalidChange(update) && a.push(item));
        }
