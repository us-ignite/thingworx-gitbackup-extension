...
function createToggle(cell, config, toggle, hideToggle, create) {
    ...
    if (!hideToggle) {
        ...
        icon.setAttribute('size', 'custom');
        icon.style.flex = '0 0 auto';
        icon[fieldToggle] = toggle;

        icon.addEventListener('mousedown', activateToggle);
        icon.addEventListener('touchstart', activateToggle);
    ...
    body.performGridDisable = function() {}; // Do nothing

    body.assignGridStateData = function(type, dep, name) {
        if (type === 'row') {
            body.lastElementChild._depfield = dep;
        } else if (type === 'column') {
            body.lastElementChild.setAttribute('state-key', name);
            body.lastElementChild._depcolumn = dep;
        }
    };
...
export function uiTreeToggle(uiControl, opt) {
    const {create, assign, format, externalEdit} = uiControl;
