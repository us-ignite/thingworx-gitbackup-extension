...
export const BehaviorChart = superClass => class extends superClass {
    ...
    static get _leftActions() {
        return [
            // Select All / Unselect All
            {
                type:   'link',
                id:     'select-all',
                label:  '_if(_chartSelection_length, unselectAllLabel, selectAllLabel)',
    ...
    _setChartSelection(_chartSelection) {
        this._chartSelection = _chartSelection;

        if (Array.isArray(this._chartSelection)) {
            this._chartSelection_length = this._chartSelection.length;
        } else {
            this._chartSelection_length = typeof this._chartSelection === 'number' ? 1 : 0;
        }
    }
