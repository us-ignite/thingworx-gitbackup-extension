...
PTCS.ChartCoreSchedule = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    static get styles() {
        return css`
        :host {
            display: block;
    ...
    render() {
        ...
        <div id="chart" class=${this._filter()} @dragstart=${this._ondragstart}
            ><div id="sub"></div
            ><div id="dragrect" part="drag-rect"
        ></div></div>`;
    }
    ...
    _filter() {
        if (!(Array.isArray(this.filterLegend))) {
            return '';
        }
    ...
    refreshData() {
        ...
        labels.forEach(item => {
            item.sub = [... item.sub].sort((a, b) => {
                /*
                    ***
                    TW-109235 - This was actully a bug prior to the Lit porting where the _legendMap wasn't populated yet,
                    thus it used the labels as fallback and ordered it alphabetically instead of the order in the legend.
                    Commenting it out for now to prevent backwards compatibility issues, and on the next release we might
                    add a new property so we can control the ordering (alphabetically/legend).
                    ***
                ...
                */
                ...
                const k1 = `X${a}` || this._legendMap[a];
                const k2 = `X${b}` || this._legendMap[b];
    ...
    refresh() {
        this.requestUpdate(); // For backwards compatibility
    }
    ...
    _duration(date1, date2) {
        ...
        const mo1 = date1.getMonth();
        const mo2 = date2.getMonth();
    ...
    selectData(selection) {
        ...
        if (selection !== null) {
            const verify = s => typeof s === 'object' &&
                                typeof s.group === 'number' && typeof s.task === 'number' &&
                                typeof s.hasOwnProperty('label') && s.hasOwnProperty('data');
