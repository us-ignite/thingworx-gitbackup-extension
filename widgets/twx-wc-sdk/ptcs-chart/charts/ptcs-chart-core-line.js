...
const refreshChartProperties = [
    'xScale', 'yScale', 'y2Scale', 'hideLines', 'showAreas', 'marker', 'markerSize', 'showValues', 'showY2Axis',
    'sampleSize', '_data', '_stackedData', '_data2', '_stackedData', 'flipAxes',
    'curve', 'bundleBeta', 'cardinalTension', 'catmullRomAlpha', 'stepPosition'
];


PTCS.ChartCoreLine = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    ...
    render() {
        return html`
        <div id="chart" class=${this._filter()} @dragstart=${this._ondragstart}>
            <ptcs-part-observer>
            <svg @click=${this._clickOnChart} @mousedown=${this._dragStart} @mousemove=${this._moveOverChart}>
                <defs id="defs">
                ...
                <g class="nstack" ?hidden=${this.hidenstack}>
                    <g id="areas"></g>
                    <g id="lines"></g>
                    <g id="markers"></g>
                </g>
                <g class="stack1" ?hidden=${this.hidestack1}>
                    <g id="areas1"></g>
                    <g id="lines1"></g>
                    <g id="markers1"></g>
                </g>
                <g class="stack2" ?hidden=${this.hidestack2}>
                    <g id="areas2"></g>
                    <g id="lines2"></g>
                    <g id="markers2"></g>
                </g>
            ...
            <div id="values" ?hidden=${this._hideValues(this.hidenstack)}></div>
            <div id="values1" ?hidden=${this._hideValues(this.hidestack1)}></div>
            <div id="values2" ?hidden=${this._hideValues(this.hidestack2)}></div>
        </div>`;
    }
    ...
    static get properties() {
        return {
            ...
            // Format specifier for values associated to primary y-axis (number or date)
            yChartValueFormat: {
                type: String
            },

            // Format specifier for values associated to secondary y-axis (number or date)
            y2ChartValueFormat: {
                type: String
            },
    ...
    static get observers() {
        return [
            '_change(data, stackMethod, stackOrder, _xCmp, _yCmp, yType, yChartValueFormat)',
            '_change2(data2, stackMethod2, stackOrder2, _xCmp, _y2Cmp, y2Type, y2ChartValueFormat)',
    ...
    constructor() {
        ...
        this._selectionMgr = new SelectionMgr(compareSelectionObjects);
        this._selectionMgr.observe(this);

        this._$formatChartValue = PTCS.formatNumber();
        this._$format2ChartValue = PTCS.formatNumber();
    }
    ...
    ready() {
        ...
        enableSvgGradients(this, this.$.defs); // $.defs is not ready in first connectedCallback...
    }

    willUpdate(changedProperties) {
        super.willUpdate(changedProperties);

        if (changedProperties.has('yChartValueFormat')) {
            // Formatting function for numeric values associated with primary y-axis
            this._$formatChartValue = PTCS.formatNumber(this.yChartValueFormat);
        }

        if (changedProperties.has('y2ChartValueFormat')) {
            // Formatting function for numeric values associated with secondary y-axis
            this._$format2ChartValue = PTCS.formatNumber(this.y2ChartValueFormat);
        }
    }
    ...
    connectedCallback() {
        ...
        const defs = this.shadowRoot.getElementById('defs');
        if (defs) {
            enableSvgGradients(this, defs);
        }
    }
    ...
    _filterLegendChanged() {
        if (this._stackedData) {
            this._change(this.data, this.stackMethod, this.stackOrder, this._xCmp, this._yCmp, this.yType, this.yChartValueFormat);
            this._change2(this.data2, this.stackMethod2, this.stackOrder2, this._xCmp, this._y2Cmp, this.y2Type, this.y2ChartValueFormat);
        }
    ...
    _filter() {
        ...
        return 'filter ' + this.filterLegend.map(item => `L${item + 1}`).join(' ');
    }
    ...
    _hideValues(hide) {
        return hide || (this.showValues !== 'above' && this.showValues !== 'on' && this.showValues !== 'below');
    }
    ...
    _change2(/* data2, stackMethod, stackOrder, _xCmp, _y2Cmp, y2ChartValueFormat */) {
        if (this.__changeOn2) {
            return;
        }
    ...
    updated(changedProperties) {
        ...
        if (refreshChartProperties.some(propName => changedProperties.has(propName))) {
            this.__refreshChart();
        }
    }
    ...
    __filterData() {
        ...
        const process = (_data, yScale, deltaY, mapSerie, isY2data) => {
                    ...
                    if (x !== undefined && 0 <= x && x <= maxX) {
                        ...
                        if (y !== undefined) {
                            if (point[2] !== undefined) { // depfield?
                                _serie.push([x + deltaX, y + deltaY, pix, point[1], point[2], isY2data]);
                            } else {
                                _serie.push([x + deltaX, y + deltaY, pix, point[1], undefined, isY2data]);
                            }
                        }
                    }
                });
        ...
        if (this.showY2Axis && y2Scale && this.data2 && this._data2) {
            ...
            process(this._data2, y2Scale, deltaY2, mapSerie2, true);
        }
    ...
    __refresh() {
        ...
        if (!this._hideValues()) {
            ...
            function formatLabel(value, y2) {
                if (value instanceof Date) {
                    return moment(value).format(y2 ? that.y2ChartValueFormat : that.yChartValueFormat);
                } else if (typeof value === 'number') {
                    return y2 ? that._$format2ChartValue(value) : that._$formatChartValue(value);
                }
                return __dataLabel(value);
            ...
            function processLabel(d) {
                ...
                if (d[5] && that.y2ChartValueFormat) { // d[5] true when value is associated to second y-axis
                    this.label = formatLabel(d[3], true);
                } else if (d[5] === undefined && that.yChartValueFormat) {
                    this.label = formatLabel(d[3]);
                } else {
    ...
    _moveOverChart(ev) {
        ...
        this.__hoverPickY = y;
        this.__onChart = true;
        if (!this._hoverPickReqAnimID) {
            this._hoverPickReqAnimID = requestAnimationFrame(() => {
                this._hoverPickReqAnimID = undefined;
                if (this.__onChart) {
    ...
    _translateIndex(data, valueIx) {
        let index = 0, validIndex = 0;
        for (index = 0; index < data.length; index++) {
            if (data[index][0] !== undefined) {
                if (validIndex === valueIx) {
                    break;
                }
                validIndex++;
            }
    ...
    _tooltipText(valueIx, serieIx) {
        ...
        const _data = (s.data === this.data2) ? (this._data2?.[serieIx] || this._stackedData2?.x) : (this._data?.[serieIx] || this._stackedData?.x);
        const v = s.data[s.data.length !== _data?.length ? this._translateIndex(s.data, valueIx) : valueIx];
    ...
    _pickData(valueIx, serieIx, findAvailableSerie = false) {
        ...
        const yScale = isY2 ? this.y2Scale : this.yScale;
        if (!yScale) {
            return null;
        }
        let point;
    ...
    _checkFilteredSeries(current, next, limit) {
        ...
        const operation = nextEQLimit === currentBIGnext ? 1 : -1;
    ...
    _keyDown(ev) {
        ...
        const six = series.findIndex(item => item === this._focus.serieIx);
    ...
    _findMarker({valueIx, serieIx}) {
        const selector = `.marker-legend[legend=L${serieIx + 1}] > [x-index="${valueIx}"]`;

        return (!this.hidenstack && this.$.markers.querySelector(selector)) ||
            (!this.hidestack1 && this.$.markers1.querySelector(selector)) ||
            (!this.hidestack2 && this.$.markers2.querySelector(selector));
    }
