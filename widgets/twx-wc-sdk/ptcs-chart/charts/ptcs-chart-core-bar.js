...
const methodConverter = {
    fromAttribute: value => value ? value : undefined,
    toAttribute:   value => value ? value : undefined
};

PTCS.ChartCoreBar = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    static get styles() {
        return css`
        :host {
            width: 100%;
    ...
    render() {
        return html`<div id="bars" class=${this._filterBar()}
             @mousemove=${this._mouseTooltip} @mouseout=${this._leaveBars} @click=${this._clickOnBars}
             @mousedown=${this._mouseDown} @dragstart=${this._ondragstart}
    ...
    _stacked(isY2 = false) {
        ...
        return stackMethod && stackMethod !== 'grouped';
    }
    ...
    _computeMinMax(isY2 = false) {
        ...
        if (isY2) {
            this.setProperties({y2Min: yMin, y2Max: yMax});
        } else {
            this.setProperties({yMin, yMax});
        }
    }
    ...
    _analyzeData(isY2 = false) {
        if (!Array.isArray(isY2 ? this._data2 : this._data)) {
            return;
        }
    ...
    refreshData(isY2 = false) {
        this.requestUpdate(isY2 ? 'data2' : 'data');
    }

    _clearChart() {
        const dragrect = this.shadowRoot.getElementById('dragrect');
        const bars = this.shadowRoot.getElementById('bars');
        const aggr = this.shadowRoot.getElementById('aggr');

        if (dragrect) {
            dragrect.remove();
        }
        if (bars) {
            this._clearElement(bars);
            if (dragrect) {
                bars.appendChild(dragrect);
            }
        }
        if (aggr) {
            this._clearElement(aggr);
        }
    }

    _filterBar() {
        return Array.isArray(this.filterLegend) ? `filter ${this.filterLegend.map(item => `L${item + 1}`).join(' ')}` : '';
    }
    ...
    willUpdate(changedProperties) {
        ...
        if (changedProperties.has('showY2Axis')) {
            this._clearChart();
            this._computeMinMax(true);
        }

        if (changedProperties.has('flipAxes') && this.flipAxes) {
            this.rotateValues = false;
            this.hideLabels = false;
        }

        if (changedProperties.has('legend')) {
            this._computeLegend(this.legend);
        }
        ...
        const stackMethod = changedProperties.has('stackMethod');
        const stackMethod2 = changedProperties.has('stackMethod2');
        let filterLegend = changedProperties.has('filterLegend');

        if (stackMethod || stackMethod2 || filterLegend) {
            this._clearChart();

            if (stackMethod) {
                this.yAxisNumberFormat = ''; // Why?
            }
            if (stackMethod2) {
                this.y2AxisNumberFormat = ''; // Why?
            }
        }
        ...
        const f = isY2 => {
            ...
            this[isY2 ? '_data2' : '_data']  = this[isY2 ? 'data2' : 'data']; // No data massage at this time
            this._analyzeData(isY2);
        };

        if (stackMethod || changedProperties.has('data')) {
            f();
        }

        if (stackMethod2 || changedProperties.has('data2')) {
            f(true);
        }
    ...
    _mapToDOM(isY2) {
        ...
        const showOutside = !isStacked || numSeries <= 1; // Can bar values appear outside of bar?
        const scaleValue = isY2 ? (this.showY2Axis && this.y2Scale) : this.yScale;
        ...
        if (reverseYAxis) {
            if (this.flipAxes) {
                ...
                if (isStacked) {
                    ...
                    arg.barLength = d => d === undefined ? '' : `${barLength(d)}px`;
    ...
    __d3_horz({scaleLabel, scaleLegend, bandWidth, bandWidth2, topValue, barLength, valueTranslate, maxLabelLength, isY2}) {
        ...
        const barText = d => this._$formatChartValue(d instanceof Array ? d[1] : d);
    ...
    __d3_horz_stack({scaleValue, scaleLabel, scaleLegend, bandWidth, topValue, valueTopPos, valueBottomPos, valueTopNeg, valueBottomNeg, isY2, valueTranslate}) {
        ...
        const barText = d => (d[4] === undefined ? 'N/A' : this._$formatChartValue(d[4]));
        const aggrText = d => this._$formatChartValue(d[1]);
    ...
    __d3_vert_stack({scaleValue, scaleLabel, scaleLegend, bandWidth, leftValue, valueLeftPos, valueRightPos, valueLeftNeg, valueRightNeg, isY2, valueTranslate}) {
        ...
        const topOffset = scaleLegend(isY2 ? 'stacked2' : 'stacked1') || 0;
        const barTop = d => `${topOffset + scaleLabel(d[1])}px`;
    ...
    _checkHorizontalLabels(barHeight) {
        ...
        const isValueOutsideBar = this.showValues === 'outside' && !this._stacked();
        ...
        requestAnimationFrame(() => {
            ...
            for (let i = list1.length - 1; i >= 0; i--) {
                const el = list1[i];
                const overflows = (!isValueOutsideBar && el.firstChild.clientWidth > el.parentNode.clientWidth) ||
                el.firstChild.clientHeight > barHeight;
                el.style.visibility = overflows ? 'hidden' : '';
            }
