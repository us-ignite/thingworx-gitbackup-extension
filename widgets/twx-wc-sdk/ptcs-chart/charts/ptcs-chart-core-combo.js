...
const compareSelectionObjects = (sel1, sel2) => (sel1.valueIx !== sel2.valueIx) ? (sel1.valueIx - sel2.valueIx) : (sel1.serieIx - sel2.serieIx);

PTCS.ChartCoreCombo = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    ...
    render() {
        return html`
        <div id="chart" @dragstart= ${this._ondragstart}>
            <ptcs-part-observer>
            <svg id="svg" @mousedown=${this._dragStart} @mousemove=${this._moveOverChart} @mouseleave=${this._mouseLeaveChart}>
                <defs id="defs">
    ...
    static get properties() {
        return {
            ...
            // Scales that maps y-positions to y-axis
            yScales: {
                type: Object
            },
    ...
    ready() {
        ...
        this._setChartStateDataEmpty(!Array.isArray(this.data) || this.data.length <= 0);
    }
    ...
    updated(changedProperties) {
        ...
        if (['drawables', 'xScale', 'yScales', 'flipAxes', 'showChartBands'].some(propName => changedProperties.has(propName))) {
            this._refresh();
        }
    }
    ...
    // Legacy method to request an update
    refresh() {
        this.requestUpdate();
    }
    ...
    _showTooltip(tooltips, x, y) {
        ...
        const displayNow = () => {
            ...
            const extractTooltip = item => {
                let text;
                const series = legend(item.serieIx);
                const value = item.y;
            ...
            getTooltipOverlayElem().show(svg, tooltip, {
