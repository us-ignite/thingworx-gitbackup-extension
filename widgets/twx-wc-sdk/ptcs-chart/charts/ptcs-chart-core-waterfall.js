...
PTCS.ChartCoreWaterfall = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    static get styles() {
        return css`
        :host {
            display: block;
    ...
    static get properties() {
        return {
            ...
            // Format specifier for ticks, tooltips, and fallback format for values when yChartValueFormat is undefined
            yValueFormat: {
                type: String
            },

            // Format for y-values in the chart area
            yChartValueFormat: {
                type: String
            },
    ...
    _preprocessData(data, filterLegend) {
        ...
        // Start values
        const allYValues = data[0][1].slice();
    ...
    _refreshValues(data, xOffs, xIndex) {
        ...
        const formatNumber = PTCS.formatNumber(this.yChartValueFormat || this.yValueFormat);
    ...
    _mouseTooltip(ev) {
        ...
        const value = dataArr.length > 4 ? dataArr[4] : valueOf(dataArr);
