...
PTCS.ChartCorePie = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    ...
    render() {
        return html`
            <div id="chart" part="chart" @dragstart=${this._ondragstart}
                @mouseout=${this._closeTooltip} @click=${this._clickOnChart}>
                <ptcs-part-observer>
                    <svg>
                        <defs id="defs"></defs>
                        <g id="pie" part="pie" @mousemove=${this._mouseTooltip}></g>
                        <g id="lines"></g>
                        <g id="gfocus">
    ...
    firstUpdated() {
        ...
        enableSvgGradients(this, this.shadowRoot.getElementById('defs'));
    ...
    willUpdate(changedProperties) {
        ...
        if (['padAngle', 'startAngle', 'endAngle', 'donut', 'polar', 'valueFormatSpecifier', 'data', 'filterLegend'].some(
    ...
    updated(changedProperties) {
        ...
        if (['padAngle', 'startAngle', 'endAngle', 'donut', 'polar', 'valueFormatSpecifier', 'data', 'filterLegend',
            // all properties above trigger method _rebuildModel
            'cornerRadius', 'highlightSelection', 'showValues', 'valuePos', 'percentLabel', 'insideLabelShowHide',
            'singleInsideValueLabelType', 'selectionMode'].some(
            propName => changedProperties.has(propName))) {
            this._refreshView();
        }
    }
    ...
    _findValueEl(index) {
        return this.shadowRoot.getElementById('values').querySelector(`ptcs-label[legend-select=L${index}]`);
    }
    ...
    _refreshView() {
        ...
        if (this._focus) {
            this._focusOn(
                this.shadowRoot.getElementById('pie').querySelector(`[part=slice][legend-select=${this._focus.getAttribute('legend-select')}]`));
        }
    ...
    _mapModelToDOM() {
        ...
        {
            // Add slices
            const path = select(this.shadowRoot.getElementById('defs'))
                .selectAll('path')
                .data(this._data || []);
            ...
            const clip = select(this.shadowRoot.getElementById('defs'))
                .selectAll('clipPath')
                .data(this._data || []);
        ...
        // Display chart slices
        {
            ...
            const join = select(this.shadowRoot.getElementById('pie'))
        ...
        // Display values
        {
            ...
            const join = select(this.shadowRoot.getElementById('values'))
            ...
            if (this._showInsideSingleValue()) {
                ...
                this.shadowRoot.getElementById('values').append(this._insidesingleValue);
                this.shadowRoot.getElementById('values').append(this._insidesingleLabel);
            } else {
        ...
        // Display lines
        {
            ...
            const join = select(this.shadowRoot.getElementById('lines'))
                .style('transform', `translate(${width / 2}px,${height / 2}px)`)
                .selectAll('line')
    ...
    _mouseTooltip(ev) {
        ...
        if (ev.clientX === undefined && ev.clientY === undefined) {
            ...
            const bb1 = this.shadowRoot.getElementById('pie').getBoundingClientRect();
            ...
            const legend = el.getAttribute('legend-select');
            const valueLabel = this.shadowRoot.getElementById('values').querySelector(`ptcs-label[legend-select=${legend}]`);
            ...
            const x2 = intersectPoint.x + Math.sin(angle) * TOOLTIP_LINE_LENGTH;
            const y2 = intersectPoint.y - Math.cos(angle) * TOOLTIP_LINE_LENGTH;
    ...
    _updateInsideLabel(index) {
        ...
        this._insidesingleValue.performUpdate();
        this._insidesingleLabel.performUpdate();
    ...
    _focusOn(focus) {
        const el = this.shadowRoot.getElementById('focus');
        this._focus = focus;
        if (focus) {
            this.shadowRoot.getElementById('gfocus').style.transform = this.shadowRoot.getElementById('pie').style.transform;
    ...
    _notifyFocus() {
        if (!this._focus) {
            this._focusOn(this.shadowRoot.getElementById('pie').querySelector('[part=slice]'));
        }
        if (this._focus) {
    ...
    _keyDown(ev) {
        ...
        const firstSlice = () => {
            return this.shadowRoot.getElementById('pie').querySelector('[part=slice]');
        };

        const lastSlice = () => {
            const slices = this.shadowRoot.getElementById('pie').querySelectorAll('[part=slice]');
    ...
    selectedChanged(index, selected) {
        ...
        const elSlice = this.shadowRoot.getElementById('pie').querySelector(`[part=slice][legend-select=L${index + 1}]`);
        const elValue = this._findValueEl(index + 1);
        const elLine = this.shadowRoot.getElementById('lines').querySelector(`[legend-select=L${index + 1}]`);
