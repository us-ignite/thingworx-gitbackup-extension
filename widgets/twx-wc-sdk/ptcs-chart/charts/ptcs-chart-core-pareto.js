...
/* eslint-disable no-nested-ternary, no-inner-declarations */
...
PTCS.ChartCorePareto = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    static get styles() {
        return css`
        :host {
            display: block;
    ...
    render() {
        return html`
        <div id="chart" @dragstart=${this._ondragstart} @mouseout=${this._closeTooltip}
             @mousemove=${this._mouseTooltip} @click=${this._clickOnChart} @mousedown=${this._dragStart}>
            <div id="bars"></div>
            <svg>
                <defs>
    ...
    constructor() {
        ...
        // this._thresholdIndex: Number

        // For tooltips
        this.labelUnits = '';
        this.labelOfTotal = 'of total';
        this.labelCumulativeValue = 'Cumulative value';
        // this.tooltipTemplate: String
        // this.tooltipTemplate2: String
        // this.legend: Array - Names of legend items, for tooltip
        // this. _yTotal: Number - Sum of all y-values on chart (affected by legend filtering)


        this._selectionMgr = new SelectionMgr(compareSelectionObjects);
        // this._unselectableSet: Set(unselectable)
    ...
    _change(data, stackOrder, filterLegend) {
        ...
        try {
            const pp = this._preprocessData(data, stackOrder, filterLegend);
            if (pp) {
    ...
    willUpdate(changedProperties) {
        ...
        if (['data', 'stackOrder', 'filterLegend'].some(propName => changedProperties.has(propName))) {
            this._change(this.data, this.stackOrder, this.filterLegend);
        }

        if (changedProperties.has('thresholdValue')) {
            this._thresholdValueChanged(this.thresholdValue);
        }

        if (['selectionMode', 'zoomSelect'].some(propName => changedProperties.has(propName))) {
            this._selectionModeChanged(this.selectionMode, this.zoomSelect);
        }

        if (changedProperties.has('unselectable')) {
            this._unselectableChanged(this.unselectable);
        }
    }
