...
export class DrawBase {
    ...
    drawMarkers({el, xScale, yScale, flipAxes, selectionMgr}) {
        ...
        const depField = i => this._data[i][2] && _hasValue(this._data[i][2][this._seriesIx]) ? this._data[i][2][this._seriesIx] : this._data[i][1][this._seriesIx];
    ...
    drawValues({el, xScale, yScale, flipAxes, fit}) {
        ...
        const legend = this._legend;
        const stateKey = this._seriesIx + 1;
        const depField = i => data[i][2] && _hasValue(data[i][2][this._seriesIx]) ? data[i][2][this._seriesIx] : data[i][1][this._seriesIx];
