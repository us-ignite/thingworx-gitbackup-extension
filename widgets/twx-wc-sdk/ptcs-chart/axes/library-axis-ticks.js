...
PTCS.formatDate = specifier => {
    return specifier
        ? date => specifier.split('\\n').map(format => moment(date).format(format)).join('\n')
        : formatDateDefault;
};
...
PTCS.formatNumber = specifier => {
    ...
    if (specifier) {
        ...
        return typeof Number.prototype.format === 'function'
            ? num => typeof num === 'number' ? num.format(specifier) : num
            : num => typeof num === 'number' ? num.toFixed(numDecimals) : num;
    }
...
export function createTicks(scale, type, maxTicks, formatSpecifier, showZero) {
    ...
    // Do we need to add a zero to a numeric range of ticks that contains zero?
    if (showZero && type === 'number' && ticks.indexOf(0) < 0 && Math.sign(ticks[0]) !== Math.sign(ticks[ticks.length - 1])) {
        ticks.unshift(0);
    }
