...
| grouping                | String   | Enables grouping. Selects a string from each item in _items_ to be used as a grouping key. If `grouping` is a _string_, it specifies a field in the item that contains the grouping key. If `grouping` is a _function_, the function will be called with each item and should return the grouping key. |         |
...
| externalStayUnvalidated | Boolean   | `_stayUnvalidated` is set _externally_ instead of by the list itself   |                      |
| validationMessageIsListItem | Boolean | Insert the validation message as a list item always, also for empty list?  |                |
