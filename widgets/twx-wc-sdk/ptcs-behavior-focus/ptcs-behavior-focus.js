...
import {getFocusHiliteEl} from 'ptcs-library/library-focus.js';
...
function deepFocus() {
    ...
    // Follow focus through all shadow doms
    for (;;) {
...
function getTabScopes(el) {
    ...
    for (let e = el; e;) {
        ...
        } else if (parentNode.nodeType === 11 && parentNode.host) {
            if (parentNode.host.tabIndex >= 0 || isWcWithFocusable(parentNode.host)) {
...
export function getFocusable(scope) {
    ...
    const tabindex = e => e.tabIndex > 0 ? e.tabIndex : 32768;
...
export function getLastFocusable(el) {
    ...
    return !(el instanceof HTMLSlotElement) && el.tabIndex >= 0 && el;
}

// Go down to the first focusable element
export function getFirstFocusable(el) {
    if (el.tabIndex >= 0) {
        return el; // el itself if focusable
    }

    const sub = ((el instanceof HTMLSlotElement) || el.shadowRoot) ? getFocusable(el) : [];

    for (let i = 0; i <= sub.length - 1; i++) {
        const e = getFirstFocusable(sub[i]);
        if (e) {
            return e;
        }
    }
...
function selectElement(el) {
    return function(acc, e, index, array) {
        if (e === el) {
        ...
        // e contains el, so we have a kind of a match
        if (acc === undefined) {
            return index;
        }
        return array[acc].contains(e) ? index : acc;
    };
}

// Find the tabbable element that precedes el
export function getPrevFocusable(el) {
    const scopes = getTabScopes(el);

    for (let i = 0; i < scopes.length; i++) {
        const tabOrder = getFocusable(scopes[i]);
        const currEl = i ? scopes[i - 1] : el;
        const fi = tabOrder.reduce(selectElement(currEl), undefined);
        ...
        if (fi > 0) {
            for (let j = fi - 1; j >= 0; j--) {
                const e = getLastFocusable(tabOrder[j]);
                if (e) {
                    return e;
                }
            }
...
export function delegateToPrev(el) {
    const e = getPrevFocusable(el || deepFocus());
...
// Find the tabbable element that follows el
export function getNextFocusable(el, skipDescendants) {
    if (!skipDescendants && el.tabIndex >= 0 && isWcWithFocusable(el)) {
        const tabOrder = getFocusable(el);
        for (let i = 0; i < tabOrder.length; i++) {
            const fe = getFirstFocusable(tabOrder[i]);
            if (fe) {
                return fe;
            }
        }
    ...
    for (let i = 0; i < scopes.length; i++) {
        ...
        const currEl = i ? scopes[i - 1] : el;
        const fi = tabOrder.reduce(selectElement(currEl), undefined);
        if (fi < tabOrder.length - 1) {
            for (let j = fi + 1; j <= tabOrder.length - 1; j++) {
                const e = getFirstFocusable(tabOrder[j]);
                if (e) {
                    return e;
                }
            }
...
export function delegateToNext(el, skipDescendants) {
    const e = getNextFocusable(el || deepFocus(), skipDescendants);
    if (e && typeof e.focus === 'function') {
        e.focus();
        return true;
    }
...
function focusEv(ev) {
    ...
    if (typeof el._notifyFocus === 'function') {
        el._notifyFocus(ev.defaultPrevented);
    }
