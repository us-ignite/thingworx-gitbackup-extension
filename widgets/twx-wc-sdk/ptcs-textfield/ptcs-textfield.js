...
PTCS.Textfield = class extends PTCS.BehaviorTabindex(PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(
    ...
    static get styles() {
        ...
        :host(:hover:not([disabled]):not([read-only])[show-clear-text][has-text]) [part=clear-button] {
          display: inline-flex;
        }

        :host(:not([disabled]):not([read-only])[show-clear-text][has-text]) #maskShell[focused]+[part=clear-button] {
          display: inline-flex;
        }

        /* E.g. the datepicker needs to show the clear button DESPITE readonly */
        :host(:hover:not([disabled])[show-clear-text][has-text][display-clear-button-on-readonly]) [part=clear-button] {
          display: inline-flex;
        }

        :host(:not([disabled])[show-clear-text][has-text][display-clear-button-on-readonly]) #maskShell[focused]+[part=clear-button] {
          display: inline-flex;
        }
        ...
        /* Chromium Edge fix to remove auto-generated reveal password eye icon in password input field, see
           https://learn.microsoft.com/en-us/microsoft-edge/web-platform/password-reveal#visibility-of-the-control
        */
        [part=text-value]::-ms-reveal {
          display: none;
        }`;
    }
    ...
    render() {
        ...
        <div part="root">
            ...
            <div part="text-box" id="textbox">
                <ptcs-icon part="icon"
                    .disabled=${this.disabled} .icon=${this.icon} .iconSet=${this.iconSet} .size=${this._iconSize()}
                ...
                <span id="maskShell" class="shell">
                    ...
                    data-hint-text=${this.dataHintText} .value=${this._getText()} @input=${this.onInput} title=""
    ...
    static get properties() {
        return {
            ...
            // Deprecated. Adds a clear-text button at the end of the field
            showClearText: {
                type:        Boolean,
            ...
            // Hides the clear-text button at the end of the field
            hideClearText: {
                type:        Boolean,
                value:       false,
            ...
            // Aligns the label to the right, left, or center.
            labelAlignment: {
                type:      String,
                value:     'left',
            ...
            // Sets the text alignment to left or right.
            textAlignment: {
                type:      String,
                value:     'left',
            ...
            // Specifies the icon with an icon set
            icon: {
                type:    String,
                reflect: true
            },
            ...
            // A URL to an SVG icon set from which to select an icon (identified via `icon`)
            iconSet: {
                type:      String,
                attribute: 'icon-set',
                reflect:   true
            ...
            // Specifies the mask of the following format: a-alpha, 9-numeric, *-alphanumeric
            mask: {
                type:     String,
                reflect:  true,
...
*/
    ...
    ready() {
        ...
        this.$.clearbutton.addEventListener('mousedown', () => {
            const focus = PTCS.hasFocus(this.$.input);
            this.__pressed = true;
            document.addEventListener('mouseup', () => {
                this.__pressed = undefined;
                if (focus) {
                    // this is how it used to work before TW-103627 - only turn on validation if textfield had focus when pressed
                    this._stayUnvalidated = false;
                }
            }, {once: true});
        }, {capture: true});
        ...
        this.addEventListener('mousemove', ev => this._showTooltip(ev));
        ...
        this.tooltipFunc = this.$.textbox.tooltipFunc =  () => this._getTooltip();
        this.$.textbox.tooltipIconFunc = () => this.tooltipIcon || '';
    ...
    _getText() {
        // We must allow the text field to show the number 0...
    ...
    _getTooltip() {
        ...
        if (!this.password && this.isTruncated()) {
