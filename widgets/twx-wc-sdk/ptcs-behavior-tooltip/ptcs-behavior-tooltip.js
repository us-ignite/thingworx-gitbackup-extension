...
const _ptcsFocusPath = []; // Hover tree
...
function __showTooltip(switched, showAnyway = false, _delay = undefined) {
    ...
    // HACK: If the current item is not a faked hover element, remove all faked hover element. Assume they are obsolete becase of the new tooltip
    if (!item?.fake && _ptcsFocusPath.some(_item => _item.fake)) {
        for (let i = _ptcsFocusPath.length - 1; i >= 0; i--) {
            if (_ptcsFocusPath[i].fake) {
                _ptcsFocusPath.splice(i, 1);
            }
        }
    }
    ...
    setTimeout(() => {
        ...
        const tooltipIcon = item.tooltipIcon || (typeof item.el.tooltipIconFunc === 'function' ? item.el.tooltipIconFunc() : item.el.tooltipIcon);
...
export function setTooltipByFocus(elChild) {
    ...
    requestAnimationFrame(() => {
        ...
        for (let e = __getFocusEl(__setFocusChild); e; e = __parentOf(e)) {
            const el = getFocusHiliteEl && getFocusHiliteEl(e);
...
function __enterEv(ev) {
    ...
    // Is this a faked event that may not be followed by __leaveEv, because the mouse might not actually hover the element?
    const fake = !(ev instanceof Event);
    ...
    _ptcsFocusPath.push({el, tooltip, tooltipIcon: ev.tooltipIcon, clientX: ev.clientX, clientY: ev.clientY, arg: tArg, fake});
...
export function hoverTooltip(el, clientX, clientY, tooltip, tooltipIcon) {
    __hideTooltip();
    __enterEv({target: el, clientX, clientY, tooltip, tooltipIcon});
}
...
PTCS.BehaviorTooltip = superClass => {
    return class extends superClass {
        ...
        _tooltipEnter(el, clientX, clientY, tooltip, arg, tooltipIcon) {
            __enterEv({target: el, clientX, clientY, tooltip, tooltipIcon, tooltipArg: arg});
        }
