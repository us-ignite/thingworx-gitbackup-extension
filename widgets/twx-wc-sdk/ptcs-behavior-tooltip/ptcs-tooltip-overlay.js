...
PTCS.__TooltipOverlay = class extends PTCS.BehaviorStyleable(L2Pw(LitElement)) {
    static get styles() {
        return css`
        :host {
            position: fixed;
    ...
    render() {
        ...
        <div class="hbar" part="tooltip">
            ...
            <ptcs-icon id="icon" part="tooltip-icon" icon="${this.tooltipIcon}" size="small" ?hidden="${!this.tooltipIcon}"></ptcs-icon>
    ...
    _setTopAttribute(_top) {
        if (_top === 'hide') {
            this.setAttribute('_top', 'hide');
        } else {
            PTCS.setbattr(this, '_top', _top);
        }
    }
    ...
    show(el, tooltip, area, showAnyway, variant) {
        ...
        if (tooltip) {
            ...
            let _tooltipIcon = (area && area.tooltipIcon) || el.tooltipIcon;
            if (typeof _tooltipIcon === 'function') {
                _tooltipIcon = _tooltipIcon.call(el) || '';
            }
            this.tooltipIcon = _tooltipIcon;
    ...
    _placeWindow(el, area) {
        ...
        const shiftX = areaArg.shiftX || 0;
        const shiftY = areaArg.shiftY || 0;
    ...
    _drawTooltipLine(tooltipLine, pos, width, height) {
        ...
        const x1 = 0;
        const y1 = 0;
        const {angle, length} = tooltipLine;
    ...
    _setPos(x, y, top, arg, p) {
        this._setTopAttribute(arg.hidePointer ? 'hide' : top);
