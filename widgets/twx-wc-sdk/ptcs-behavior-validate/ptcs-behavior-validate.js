...
import 'ptcs-library/lit-polymer-polyfill.js';
...
const messageProperties = [
    'validationSuccessIcon', 'validationErrorIcon', 'validationCriteriaIcon', 'validationMessage',
    'validationSuccessMessage', 'validationCriteria', 'validationSuccessDetails'
];

const attributeProperties = ['hideValidationSuccess', 'hideValidationError', 'hideValidationCriteria'];
...
PTCS.BehaviorValidate = superClass => {
    return class extends superClass {
        ...
        constructor() {
            ...
            this._externalMsgs = {};
            this.__validationPromiseController = {};

            this.validationTimeout = 10;
            this._validationChangeNo = 0;

            // This would save us from many change-in-update warnings, but it unfortunately causes some unit tests to fail
            // this._stayUnvalidated = true; // By default, stay unvalidated until user interacts with control

            // Information for whoever that wants to listen
            this._validationMessageResizeObserver = new ResizeObserver(() => requestAnimationFrame(() => {
                this._noSpaceForMessage = this.offsetHeight < this.scrollHeight;
            }));
        }

        _initValidationBehavior() {
            // Add observers manually: cannot assume that the L2Pw machinery is available for LitElements
            const _markValidity = this._markValidity.bind(this);
            const _validityChanged = this._validityChanged.bind(this);
            const _messageFormatChanged = this._messageFormatChanged.bind(this);
            const _validityAttributeChanged = this._validityAttributeChanged.bind(this);
            const _validationOutputChanged = this._validationOutputChanged.bind(this);

            this._debouncedMessageFormat = PTCS.debounce(_messageFormatChanged, MARK_VALIDITY_TIMEOUT);

            this._createPropertyObserver('externalValidity', _markValidity);
            this._createPropertyObserver('_stayUnvalidated', _markValidity);
            this._createPropertyObserver('validity', _validityChanged);
            messageProperties.forEach(propName => this._createPropertyObserver(propName, _messageFormatChanged));
            attributeProperties.forEach(propName => this._createPropertyObserver(propName, _validityAttributeChanged));
            this._createPropertyObserver('validationOutput', _validationOutputChanged);
        ...
        ready() {
            ...
            // Hack to initialize non Lit elements
            if (!(this instanceof LitElement)) {
                // Probably an unmigrated Polymer element
                this._initValidationBehavior();
            }
        }
        ...
        _validityAttributeChanged() {
            this._messageFormatChanged();
            this._setValidityAttribute();
        }

        _validationOutputChanged() {
            this.dispatchEvent(new CustomEvent('validation-output-changed', {detail: {value: this.validationOutput}}));
        }

        _validityChanged() {
            this.__validityChanged(this.validity);
            this._setValidityAttribute();
            this.dispatchEvent(new CustomEvent('validity-changed', {detail: {value: this.validity}}));
        }
            ...
            for (let el = this; el; el = Object.getPrototypeOf(el)) {
                ...
                for (const propName in properties) {
                    ...
                    if (!stop[propName]) {
                        ...
                        this.__parseValidate(propName, validate, properties[propName]);
        ...
        __parseValidate(propName, validate, propDecl) {
            ...
            this.constructor.__validation[propName] = {method, args, indirect: propDecl.indirect};
        }
        ...
        __attachValidationObservers() {
            ...
            Object.keys(this.constructor.__validation).forEach(propName => {
                ...
                if (this.constructor.__validation[propName].indirect) {
                    // This property is marked as indirect. It can change behind the back of the setter. We therefore need to listen to
                    // the change-event, which must be generated whenever the property changes. The event might however be late, so
                    // we need to keep listening to the setter. The changed event is only needed as a backup, for missed setter changes.
                    let oldValue = this[propName];
                    const changed = () => {
                        const value = this[propName];
                        if (oldValue !== value) {
                            oldValue = value;
                            this.__validationPropertyChanged(propName);
                        }
                    };

                    this.addEventListener(`${window.camelToDashCase(propName)}-changed`, changed);
                    this._createPropertyObserver(propName, changed);
                } else {
        ...
        __updateConstraints(set) {
            ...
            for (const propName in validation) {
                ...
                if (this.__hasValidationArgs(propName)) {
                    ...
                    this._validatedProps[propName] = NOT_VALIDATED;
                    this._setPropMsg(propName); // Clear property message
                }
            }
        ...
        _setPropMsg(propName, msg) {
            if (this._externalMsgs[propName] !== msg) {
                this._externalMsgs[propName] = msg;
                this._debouncedMessageFormat(); // Manual bump
            }
        }
        ...
        _updateValidity(propName) {
            ...
            this._validatedProps[propName] = VALIDATED; // Need to validate property. Reset old validation result
                            ...
                            switch (val) {
                                ...
                                default:
                                    if (Array.isArray(val)) {
                                        this._validatedProps[propName] = VALIDATED_FALSE;
                                        propMessage = val.filter(msg => msg);
                                    } else {
                                        this._validatedProps[propName] = VALIDATED_IGNORE;
                                        console.warn(`${validateMethod} on ${this.tagName} should return true | false | array | undefined`);
                                    }
                            }
                            this._setPropMsg(propName, propMessage && propMessage.length ? propMessage : undefined);
                            this._markValidity();
                        })
                        .catch((e) => {
                            if (e === timeoutError) {
        ...
        _doMarkValidity() {
            ...
            if (this._propPending && !this._stayUnvalidated) {
                this.dispatchEvent(new Event('validation-completed'));
            }

            this._propPending = false;
        }
        ...
        __validityChanged(validity) {
            ...
            if (!validity) {
                if (this._validationMessageEl.parentNode) {
                    this._validationMessageEl.remove();
                    ...
                    this._removedValidationMessage();
                }
            } else {
                ...
                this._validationMessageEl.requestUpdate();
            ...
            requestAnimationFrame(() => {
                ...
                if (hasFixedHeight) {
                    ...
                    if (!messageElement.hidden) {
                        // Sometimes the message gets hidden before the animation frame
