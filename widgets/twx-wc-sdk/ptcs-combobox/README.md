...
| grouping           | String   | Enables grouping in the popup list. Selects a string from each item in _items_ to be used as a grouping key. If `grouping` is a _string_, it specifies a field in the item that contains the grouping key. If `grouping` is a _function_, the function will be called with each item and should return the grouping key. |         |
| undoEditOnBlur     | Boolean | On blur, undo an on-going edit                                                                                     | false      |                         |
