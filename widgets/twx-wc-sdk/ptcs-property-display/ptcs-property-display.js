...
PTCS.PropertyDisplay = class extends PTCS.BehaviorTabindex(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    ...
    static get styles() {
        return css`
            ...
            :host(:not([table-layout])) [part=value-display-item] {
                height: 100%;
                width: 100%;
            }

            :host(:not([key-column-width])[table-layout]) [part~=value-display-item] {
                width: 50%;
            }

            :host([key-column-width][table-layout]) [part~=table-key] {
                width:  var(--ptcs-property-table-key-width);
            }

            :host([key-column-width][table-layout]) [part~=table-value] {
                width: var(--ptcs-property-table-value-width);
            }
           ...
           :host(:not([vertical-mode])[_responsive]) [part=property-group-container] {
            ...
            :host([table-layout]) [part=property-group-container-tabular] {
                display: flex;
            }

            :host([table-layout]) [part=table-view-label] {
                width: 100%;
            }
            ...
            <div id="propertycontainer" part="property-container">
                ...
                ${map(this._getGroups(this.items, this.itemsInfo, this.selectorGroupTitle), (group) => html`
                    <div part="property-group">
                        ...
                        ${when(this.tableLayout, () => html` 
                            ${map(this._getItemsFromGroup(group, 'groupItems'), (item) => html`
                            <div part="property-group-container-tabular">
                                    <div part="value-display-item table-key">
                                        <ptcs-label
                                            part="table-view-label"
                                            label=${this._getStringFromObject(item, this.selectorItemKey)}
                                            ?disabled=${this.disabled} ?multi-line=${this.multiLine}
                                            .maxHeight=${this.valueDisplayHeight} .disclosureControl=${'ellipsis'}>
                                        </ptcs-label>
                                    </div>
                                    <ptcs-value-display
                                        property-display-item
                                        part="value-display-item table-value"
                                            .data=${this._getObjectFromObject(item, this.selectorItemValue)}
                                            .itemMeta=${this._getObjectFromObject(item, this.selectorItemMeta)}
                                            .textWrap=${this.multiLine}
                                            modal-width=${this.modalWidth}
    ...
    static get observers() {
        return [
            '_observeWidthConstraints(valueDisplayMinWidth, valueDisplayWidth, keyColumnWidth)'
        ];
    }
    ...
    updated(changedProperties) {
        ...
        if (changedProperties.has('keyColumnWidth')) {
            requestAnimationFrame(() => this._assignWidth());
        }
    }

    _assignWidth() {
        const keyColumnWidth = this.keyColumnWidth;
        const clientWidth = this.clientWidth;
        const minValueWidth = 58;

        let keyWidth = keyColumnWidth;
        let valueWidth = clientWidth - keyWidth;

        if (keyWidth > clientWidth) {
            keyWidth = clientWidth - minValueWidth;
            valueWidth = minValueWidth;
        }

        this.style.setProperty('--ptcs-property-table-key-width', `${keyWidth}px`);
        this.style.setProperty('--ptcs-property-table-value-width', `${valueWidth}px`);
    }
    ...
    _getNumVDsInGroup(groupIdx) {
        const groupEl = this._getGroupEl(groupIdx);
        if (groupEl) {
    ...
    _getValueDisplayEl(groupEl, index, focus) {
        if (!this.tableLayout) {
            return groupEl.querySelector(`ptcs-value-display:nth-of-type(${index + 1})`);
        } else if (this.tableLayout && focus) {
            return groupEl.querySelector(`div[part="property-group-container-tabular"]:nth-of-type(${index + 1})`);
        }

        const gl = groupEl.querySelector(`div[part="property-group-container-tabular"]:nth-of-type(${index + 1})`);
        return gl.querySelector('ptcs-value-display');
    }

    _getValueContainerTabularEl(groupEl, index) {
        return groupEl.querySelector(`div[part="property-group-container-tabular"]:nth-of-type(${index + 1})`);
    }
    ...
    _getValueContainerEl(groupEl, index) {
        ...
        if (valueDisplayEl._overflow) {
            const disclosureButtonEl = valueDisplayEl.shadowRoot.querySelector('[part=disclosure-button]');
            if (valueDisplayEl.overflowOption === 'disclosure') {
                // Overflow with an overflow button, focus on button
                return disclosureButtonEl;
            } else if (valueDisplayEl.overflowOption === 'showmore') {
                // Overflow with show more / show less link, focus on link');
                const linkEl = valueDisplayEl.shadowRoot.querySelector('[part=text-link]');
                if (linkEl) {
                    return linkEl;
                }
            }

            // If we get *here*, we still have to check if we are not an image that has a disclosure button even in
            // showmore or ellipsis mode...
            if (disclosureButtonEl) {
                return disclosureButtonEl;
            }
        }
        // Focus on the VD or an interactive part thereof');
        const valueContainer = valueDisplayEl.shadowRoot.getElementById('valuecontainer');
        const textfield = valueContainer.querySelector('ptcs-textfield');
        if (textfield) {
            return textfield.shadowRoot.querySelector('div[part="text-box"]');
        }
        if (this.tableLayout) {
            const valueContainerTabular = this._getValueContainerTabularEl(groupEl, index);
            return valueContainer.querySelector('ptcs-link') || valueContainerTabular;
        }
        return valueContainer.querySelector('ptcs-link') || valueDisplayEl;
    }

    _makeFocusItemVisible() {
        const pdRect = this.getBoundingClientRect();
        const bottomEnd = Math.min(pdRect.bottom, window.innerHeight);
        const topEnd = Math.max(pdRect.top, 0);
        if (this._valueDisplayFocusIndex !== -1) {
            console.assert(this._groupFocusIndex !== -1);
            const groupEl = this._getGroupEl(this._groupFocusIndex);
            const valueDisplayEl = this._getValueDisplayEl(groupEl, this._valueDisplayFocusIndex, true);
            const vdRect = valueDisplayEl.getBoundingClientRect();
            ...
            if (groupRect.top < topEnd) {
                ...
                const firstVDEl = this._getValueDisplayEl(groupEl, 0);
                if (firstVDEl) {
                    const vdRect = firstVDEl.getBoundingClientRect();
                    if (vdRect.bottom > bottomEnd) {
    ...
    _gotoNextGroup() {
        this._valueDisplayFocusIndex = -1;
        const numGroups = this._getNumGroups();
    ...
    _gotoPrevRowVD() {
        const groupEl = this._getGroupEl(this._groupFocusIndex);
        const vdElts = groupEl.querySelectorAll('ptcs-value-display');
        const numVDs = vdElts.length;

        console.assert(this._valueDisplayFocusIndex >= 0);
        console.assert(this._valueDisplayFocusIndex < numVDs);

        const currVDRect = vdElts[this._valueDisplayFocusIndex].getBoundingClientRect();
        const currTop = currVDRect.top;
        const currLeft = currVDRect.left;

        // Going up...
        if (this._valueDisplayFocusIndex > 0) {
            for (let i = this._valueDisplayFocusIndex - 1; i >= 0; i--) {
                const prevVDRect = vdElts[i].getBoundingClientRect();
    ...
    _gotoNextRowVD() {
        ...
        for (let i = this._valueDisplayFocusIndex + 1; i < numVDs; i++) {
            const nextVDRect = vdElts[i].getBoundingClientRect();
    ...
    _goRight() {
        if (this._valueDisplayFocusIndex !== -1) {
            ...
            const numVDs = this._getNumVDsInGroup(this._groupFocusIndex);
    ...
    _activate(ev) {
        if (this._valueDisplayFocusIndex !== -1) {
            ...
            const groupEl = this._getGroupEl(this._groupFocusIndex);
            if (groupEl) {
                const valueDisplayEl = this._getValueDisplayEl(groupEl, this._valueDisplayFocusIndex);
                if (valueDisplayEl) {
    ...
    _selectElementAtPoint(atX, atY) {
        const groupElts = this.$.propertycontainer.querySelectorAll('div[part="property-group"]');
        for (let i = 0; i < groupElts.length; i++) {
            if (this._inRect(atX, atY, groupElts[i].getBoundingClientRect())) {
                ...
                const vdElts = groupElts[i].querySelectorAll('ptcs-value-display');
    ...
    _initTrackFocus() {
        this._trackFocus(this, () => {
            if (this._groupFocusIndex >= 0) {
                ...
                if (groupEl) {
                    if (this._valueDisplayFocusIndex >= 0) {
                        const valueDisplayEl = this._getValueContainerEl(groupEl, this._valueDisplayFocusIndex);
                        if (valueDisplayEl) {
                            // Focus on the VD
    ...
    _observeWidthConstraints(valueDisplayMinWidth, valueDisplayWidth, keyColumnWidth) {
        if (keyColumnWidth) {
            this.style.setProperty('--ptcs-property-table-key-width', keyColumnWidth + 'px');
        } else {
            this.style.setProperty('--ptcs-property-table-key-width', '50%');
        }
    ...
    _getItemsFromGroup(item, selector) {
        ...
        if (typeof selector === 'string') {
            const resultItem = item[selector];
