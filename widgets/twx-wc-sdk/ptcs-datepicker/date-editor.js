...
const secondaryTyping = {typed: false};
...
function assignMonth(date$, value) {
    return date$.set('M', !isNaN(parseInt(value)) ? Number(value) - 1 : value); // Month are assigned as 0 - 11
}
...
function weeksOf(date$) {
    const d = moment(date$).endOf('year');
    const w = d.format('w');
...
function decodeDate(format, date, text) {
    ...
    const date$ = date ? moment(date) : moment().second(0).millisecond(0);
...
function addDigit(date$, format, index, value, digit) {
    ...
    value = secondaryTyping.typed ? field.f : value;
    secondaryTyping.typed = false;
    ...
    const complete = (field.unit !== 'y' && value[0] === '0' && !restartValue) ? true : !(more && min <= more && more <= max);
    return {value: v, complete: complete};
}
...
function correctDateUpdate(format, text, date) {
    let start = 0;
    for (let i = 0; i < format.length; i++) {
        const field = format[i];
        const end = start + length(field);
        const min = !field.unit ? -1 : domainOf(field, date)[0];
        if (text.substring(start, end) < min) {
            text = text.substring(0, start) + field.f + text.substring(end);
        }
        start = end;
    }
    return text;
...
function onBlur(ev) {
    ...
    el.text = correctDateUpdate(format, el.text, date$); // Returns a correct date without 00 (in days or months if any)
    secondaryTyping.typed = true;
...
function onKeydown(ev) {
    ...
    const stayHere = () => {
        el.setSelectionRange(start, end);
        // TW-122141: Allow auto-scroll by not preventing default processing of ArrowRight / ArrowLeft
        if (!(ev.key === 'ArrowRight' || ev.key === 'ArrowLeft')) {
            ev.preventDefault();
        } else {
            // Auto-scroll, and then restore selection
            requestAnimationFrame(() => {
                el.setSelectionRange(start, end);
            });
        }
    ...
    const changeToDefault = () => {
        if (el.text.substring(start, end) === '00') {
            el.text = el.text.substring(0, start) + field.f + el.text.substring(end);
            el.performUpdate();
        }
    };
    ...
    switch (key) {
        ...
        case 'Backspace':
        case 'Delete':
            el.text = el.text.substring(0, start) + fill(field, index, format) + el.text.substring(end);
            el.performUpdate();
            stayHere();
            return;
        ...
        default:
            if (key.length === 1) {
                ...
                // The date is not valid if it is unassigned (moment will use todays date, which may result in false positives)
                if (el[dateField] && isValid(format, date$, el)) {
                    el[dateField] = date$.toDate();
                    el.text = date$.format(format.map(s => s.f || s).join(''));
                }
