...
| hideTrigger        |Boolean |Hide trigger button? If hidden, the collapse mode cannot be changed by the user, unless _trigger_ is `panel`.| false   | -                       |
...
1. By injecting ptcs-style-unit into the DOM:
```
<ptcs-style-unit wc="PTCS-DYNAMIC-PANEL">
    [part~=panel-wrapper] {
      --ptcs-margin-top: 43px;
      --ptcs-margin-bottom: 43px;
      --ptcs-margin-left: 43px;
      --ptcs-margin-right: 43px;
    }
</ptcs-style-unit>
```


2. Using `::part()` pseudo element on the exposed parts (which are mentioned in the parts table):
```
