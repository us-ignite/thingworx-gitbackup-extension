...
PTCS.Image = class extends PTCS.BehaviorTooltip(PTCS.BehaviorStyleable(L2Pw(LitElement))) {
    static get styles() {
        return css`
            :host {
    ...
    render() {
        return html`
            <div id="root" part="image">
                <img id="img" src=${ifDefined(this._src)} ?hidden=${this._imageHidden} @load=${this._onLoad} @error=${this._onError}>
                <ptcs-label part="alt-label" .label=${this.alt} .variant=${this.labelVariant}/>
            </div>
        `;
    }
    ...
    willUpdate(changedProperties) {
        ...
        this.containerDim = this._computeContainerDim(this.size, this._src);
        this._backgroundSize = this._computeBackgroundSize(this.containerDim, this.size, this._src);
        this._widthCntr = this._computeDim(this.containerDim, this._sizeW, this._widthImg);
        this._heightCntr = this._computeDim(this.containerDim, this._sizeH, this._heightImg);
    }
    ...
    _sizeChanged(size, _src) {
        ...
        if ((typeof sizeW === 'number' || typeof sizeH === 'number') && (this._src && !PTCS.imgLoaded(this.$.img))) {
