...
PTCS.Accordion = class extends PTCS.BehaviorTabindex(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    ...
    render() {
        return html`
            ${map(this.items, (item) => html`
                <ptcs-accordion-item part="item" .item=${item} .variant=${this.variant} ._level=${this._level}
                    .icon=${item.icon} .iconWidth=${this.iconWidth} .iconHeight=${this.iconHeight}
                    .label=${item.label} .content=${item.content} ._parent=${this._getParent()}
                    .hideTrigger=${this.hideTrigger} .triggerType=${this.triggerType} .triggerAlign=${this.triggerAlign}
                    .triggerCanCollapse=${this.triggerCanCollapse} ?disabled=${this.disabled} .fillMissingIcons=${this.fillMissingIcons}
                    .multipleOpenItems=${this.multipleOpenItems} .hideIcons=${this.hideIcons}
                    @opened-changed=${this._openedChanged}></ptcs-accordion-item>
            `)}
        `;
    }
