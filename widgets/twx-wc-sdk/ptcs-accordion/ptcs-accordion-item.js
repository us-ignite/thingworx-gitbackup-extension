...
PTCS.AccordionItem = class extends PTCS.BehaviorStyleable(L2Pw(LitElement)) {
    ...
    render() {
        return html`
        <div part="header" id="header" halign=${this.triggerAlign} @click=${this.toggle} aria-role="heading" ?disabled=${this.disabled}>
            ${when(!this._noIcon(this.icon, this.fillMissingIcons, this.hideIcons), () => html`
                <ptcs-icon id="icon" part="icon" .icon=${this._getIcon(this.icon, this.fillMissingIcons)}
                    .size=${this._iconSize(this.iconWidth, this.iconHeight)} .iconWidth=${this.iconWidth}
                    .iconHeight=${this.iconHeight} aria-hidden="true"></ptcs-icon>`)}
            <ptcs-label part="label" label=${this.label} aria-role="button" aria-expanded=${this._ariaBoolean(this.opened)}
                aria-disabled=${this._ariaBoolean(this.mute)}></ptcs-label>
            ${when(!this._hideTrigger(this.hideTrigger, this.content), () => html`
                <ptcs-button no-tabindex part="trigger-button" variant="small" ?disabled=${this.disabled}
                    .icon=${this._icon(this.opened, this.triggerType)} rotate=${this._rotateIcon}
                    aria-hidden="true"></ptcs-button>`)}
        </div>
        <ptcs-collapse .opened=${this.opened}>
    ...
    static get properties() {
        return {
            ...
            triggerAlign: {
                type:      String,
                attribute: 'trigger-align'
            },

            // Can trigger collapse panel?
            triggerCanCollapse: {
                type:      Boolean,
                attribute: 'trigger-can-collapse'
            },
    ...
    static get observers() {
        ...
        return ['_updatePanel(disabled, content, multipleOpenItems, triggerType, hideTrigger, triggerAlign, triggerCanCollapse, fillMissingIcons, hideIcons, variant, iconWidth, iconHeight)'];
    }
    ...
    constructor() {
        ...
        // Init the properties that previously was set with "value:"
    ...
    ready() {
        ...
        this._updatePanel(this.disabled, this.content, this.multipleOpenItems, this.triggerType,
            this.hideTrigger, this.triggerAlign, this.triggerCanCollapse, this.fillMissingIcons, this.hideIcons, this.variant);
    }

    _updatePanel(disabled, content, multipleOpenItems, triggerType, hideTrigger, triggerAlign, triggerCanCollapse,
        fillMissingIcons, hideIcons, variant, iconWidth, iconHeight) {
        if (!this.__isReady) {
            return;
        }
        ...
        if (Array.isArray(content)) {
            ...
            // eslint-disable-next-line max-len
            el.setProperties({items: content, _parent: this, _level: nextLevel, disabled, multipleOpenItems, triggerType, hideTrigger, triggerAlign, triggerCanCollapse, fillMissingIcons, hideIcons, variant, iconWidth, iconHeight});
    ...
    _getIcon(icon, fillMissingIcons) {
        if (icon) {
            return icon;
        }
        // No icon specified, use default if so configured
        return fillMissingIcons ? 'cds:icon_image' : icon;
    }

    _noIcon(icon, fillMissingIcons, hideIcons) {
        if (hideIcons) {
