...
PTCS.Label = class extends PTCS.BehaviorTooltip(PTCS.BehaviorFocus(PTCS.BehaviorStyleable(L2Pw(LitElement)))) {
    static get styles() {
        return css`
        :host {
            display: inline-flex;
        ...
        /* This should only trigger when running Safari, the below CSS will disable */
        /* the "extra" tooltip it displays over content truncated with an ellipsis... */
        .disable-safari-tooltip::after {
            content: '';
            display: block;
        }
        ...
        :host([max-number-of-lines]) [part=label] {
            display: -webkit-box;
            -webkit-box-orient: vertical;
            -webkit-line-clamp: var(--ptcs-label-max-num-of-lines);
            overflow: hidden;
        }`;
    }
    ...
    static get properties() {
        return {
            ...
            maxNumberOfLines: {
                ...
                reflect:   true,
                observer:  '_maxNumberOfLinesChanged'
            }
        };
    }
    ...
    ready() {
        ...
        if (PTCS.isSafari) {
            this.$.label.classList.add('disable-safari-tooltip');
        }
    }
    ...
    _maxNumberOfLinesChanged(val) {
        if (!isNaN(val) && val > 0) {
            this.style.setProperty('--ptcs-label-max-num-of-lines', Number(val));
            return;
        }

        this.maxNumberOfLines = null;
        this.style.removeProperty('--ptcs-label-max-num-of-lines');
    }
