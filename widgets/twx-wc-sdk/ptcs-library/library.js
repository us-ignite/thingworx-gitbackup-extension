...
PTCS.isTouchDevice = !!('ontouchstart' in window || navigator.maxTouchPoints);
...
if (!Array.prototype.findLast) {
    Array.prototype.findLast = function(test) {
        const len = this.length;
        let value;
...
//------------------------------------------------------------------------
    ...
    function xlatePercent(value, el, vert) {
        ...
        return value * (vert ? el.parentNode.clientHeight : el.parentNode.clientWidth) / 100;
    }
...
PTCS.getStyleForElement = function(shadowRootElement, selectorText) {
    const res = [];
    const classes = shadowRootElement.styleSheets[0].rules || shadowRootElement.styleSheets[0].cssRules;
    for (let x = 0; x < classes.length; x++) {
        if (classes[x].selectorText === selectorText) {
            const styleText = classes[x].style.cssText;
            const stylesArr = styleText.split(';');
            for (let i = 0; i < stylesArr.length; i++) {
                const prop = stylesArr[i].substring(0, stylesArr[i].indexOf(':')).trim();
                if (prop && prop.length > 0) {
                    let propVal = stylesArr[i].substring(stylesArr[i].indexOf(':') + 1).trim();
...
PTCS.getRGBvalue = function(element, nonRGBColor) {
    ...
    const res = window.getComputedStyle(tempElement).color;
    ...
    getFormaterFunc: function(renderType, fieldName, listFormat) {
        ...
        if (window.TW.Renderer[renderType].renderText) {
            ...
            selectorF = function(item) {
                const value = canBeFieldName(fieldName) ? item[fieldName] : item;
            ...
            if (listFormat.formatText !== null && listFormat.formatText !== undefined) {
                const tokenRegEx = /^\[\[(.*)\]\]$/g;
                const token = tokenRegEx.exec(listFormat.formatText);
            ...
            selectorF = function(item) {
                ...
                if (!PTCS.Formatter._isEmpty(value)) {
                    if (listFormat.FormatString === 'hyperlink') {
                        const href = value.href ? value.href : value;
            ...
            if (!listFormat.FormatString || listFormat.FormatString === 'plain') {
                ...
                const collection = listFormat.FormatString === 'data' ? 'DataTags' : 'ModelTags';
                selectorF = function(item) {
                    const value = canBeFieldName(fieldName) ? item[fieldName] : item;
                    const rawVal = value;
                    if (!(rawVal instanceof Array)) {
...
PTCS.validateURL = function(url) {
    if (url && typeof url !== 'string') {
        return false; // The url can for instance be the NaV Symbol (used by the delayed loading)
    }

    // Don't allow 'javascript:' protocol. Test for: capital letters, tabs and spaces between letters.
    const patt = /^\s*j\s*a\s*v\s*a\s*s\s*c\s*r\s*i\s*p\s*t\s*:/gi;
...
PTCS.openUrl = function(operationType, url, target, options) {
    ...
    const preventTabnabbingFlags = 'noopener,noreferrer';
...
PTCS._verticalScrollbarWidth = function() {
    if (_vsbWidth === undefined) {
        const el = document.createElement('div');
        el.style.overflow = 'scroll';
        el.style.width = '200px';
        el.style.height = '200px';
        document.body.appendChild(el);
        _vsbWidth = el.offsetWidth - el.clientWidth;
        el.remove();
    }
    return _vsbWidth;
};
...
/*
 * If the provided element has a scrollbar return its width. If no element, return stock width
 */
PTCS.getVerticalScrollbarWidth = function(el) {
    return (el === undefined || (el && el.scrollHeight > el.clientHeight)) ? PTCS._verticalScrollbarWidth() : 0;
};
...
PTCS.saveValueInSession = function(name, value, local = true) {
...
PTCS.restoreValueFromSession = (name, local = true) => {
...
if (!window.WebComponents) {
    ...
    const ready = () => {
        ...
        window.dispatchEvent(new CustomEvent('WebComponentsReady', {bubbles: true}));
    };
...
PTCS.normalizeUnit = function(size) {
    switch (typeof size) {
        case 'number':
            return `${size}px`;
        case 'string':
        ...
        default:
            if (size) {
                console.assert(!size, `Unknown size type: ${JSON.stringify(size)}`);
            }
            return '';
    }
    // size is a string
    ...
    // Add unit if missing
    return m[3] ? size : `${size}px`;
};


PTCS.setAttribute = function(el, name, value) {
    if (value === undefined || value === false || value === null) {
        el.removeAttribute(name);
    } else if (value === true) {
        el.setAttribute(name, '');
...
PTCS.createElement = function(name, attributes) {
    const el = document.createElement(name);
    for (const attrName in attributes) {
        PTCS.setAttribute(el, attrName, attributes[attrName]);
    }
    return el;
};

// Escapes comma and colon in view expression. Characters ':' and ',' are replaced by '\\+' and '\\_' (respectively)
PTCS.escapeViewExprChars = function(str) {
    return typeof str === 'string' ? str.replaceAll('\\', '\\!').replaceAll(',', '\\_').replaceAll(':', '\\+') : '';
};

// Restores escaped comma and colon in view expression
PTCS.restoreViewExprChars = function(str) {
    return typeof str === 'string' ? str.replaceAll('\\+', ':').replaceAll('\\_', ',').replaceAll('\\!', '\\') : '';
};

// Returns string of comma-separated pairs of name:value items (:value part when present), where comma and colon are escaped
PTCS.encodeViewExpr = function(items) {
    return (!Array.isArray(items) || items.length === 0) ? '' : items.map(([name, value]) => {
        return value ? `${PTCS.escapeViewExprChars(name)}:${PTCS.escapeViewExprChars(value)}` : `${PTCS.escapeViewExprChars(name)}`;
    }).join(',');
};

// Returns array from view expression
PTCS.decodeViewExpr = function(expr) {
    if (!expr || typeof expr !== 'string') {
        return [];
    }
    return expr.split(',').map((item) => item.split(':').map((subitem) => PTCS.restoreViewExprChars(subitem)));
};

PTCS.debounce = function(func, delay = 50) {
    let timeout;
...
// Polymer gestures overrides synthetic clicks from touch events with a 2500 ms time-out, this function resets it
PTCS.resetMouseCanceller = resetMouseCanceller;
